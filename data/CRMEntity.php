...
class CRMEntity {
	...
	function insertIntoCrmEntity($module, $fileid = '') {
		...
		if ($this->mode == 'edit') {
			...
			// 2020/09/15 Pyae Phyo Mon Add Column in vtiger_crmentity MSCRM Ver 1.0 Start
				...
				$sql = "update vtiger_crmentity set smownerid=?, smgroupid=?,modifiedby=?,description=?, modifiedtime=?,is_update=? where crmid=?";
				$params = array($ownerid, $groupid, $current_user->id, $description_val, $adb->formatDate($date_var, true),1, $this->id);
			// 2020/09/15 Pyae Phyo Mon Add Column in vtiger_crmentity MSCRM Ver 1.0 End
			} else {
			   ...
			   // 2020/09/15 Pyae Phyo Mon Add Column in vtiger_crmentity MSCRM Ver 1.0 End	
				...
				$sql = "update vtiger_crmentity set smownerid=?, smgroupid=?,modifiedby=?, modifiedtime=?,is_update=? where crmid=?";
				$params = array($ownerid, $groupid, $current_user->id, $adb->formatDate($date_var, true),1, $this->id);
				// 2020/09/15 Pyae Phyo Mon Add Column in vtiger_crmentity MSCRM Ver 1.0 End
				}
			}
			...
			$sql = "insert into vtiger_crmentity (crmid,smcreatorid,smownerid,smgroupid,setype,description,modifiedby,createdtime,modifiedtime,source,is_create) values(?,?,?,?,?,?,?,?,?,?,?)";
			$params = array($current_id, $current_user->id, $ownerid, $groupid, $module, $description_val, $current_user->id, $created_date_var, $modified_date_var,$source,1);
			// 2020/09/15 Pyae Phyo Mon Add Column in vtiger_crmentity MSCRM Ver 1.0 End
			$adb->pquery($sql, $params);
...
//Added by Richie
	...
	function mark_deleted($id) {
		...
		if($module=='Contacts')
		{
			$record = $this->db->pquery("SELECT accountid FROM vtiger_contactdetails WHERE contactid = ?", array($id));
			$accno = $this->db->query_result($record, 0, 'accountid');

			$this->db->pquery("UPDATE vtiger_crmentity set is_update = 1 where crmid = ?", array($accno));
		}
			...
			if ($fld_table == $this->table_name) {
				...
				if ($records && $adb->num_rows($records)) {
					...
					while ($recordinfo = $adb->fetch_array($records)) {
						...
						/*2020-10-08 Thet Phyo Wai Number Blank Add MSCRM Ver 1.0 Start*/
						if($module == "Accounts" || $module == "Contacts" || $module == "Vendors"){
							if($module == "Accounts"){
							...
							if($module == "Vendors"){
								$numberField = "vendor_number";
							}
							$adb->pquery("UPDATE $fld_table SET $numberField=$fld_column WHERE $numberField = '' AND $this->table_index = ?", Array($recordinfo['recordid']));
						}
						/*2020-10-08 Thet Phyo Wai Number Blank Add MSCRM Ver 1.0 End*/
					}
