...
//							$value = CurrencyField::appendCurrencySymbol($currencyValue, $currencySymbol);
							...
							if($field->getUIType() == 71 && $fieldName == 'purchase_cost' && ($module == 'Products' || $module == 'Services')) {
								$currencyId = getPurchaseCostCurrency($recordId,$module);
								$cursym_convrate = getCurrencySymbolandCRate($currencyId);
								$currencySymbol = $cursym_convrate['symbol'];
								$row['currencySymbol2'] = $currencySymbol;
								$value = CurrencyField::convertToUserFormat($value, null, true);
							}else{
								if (!empty($value)) {
									$value = CurrencyField::convertToUserFormat($value);
					...
					if(!empty($value) && !empty($this->nameList[$fieldName]) && !empty($parentModule)) {
						...
						// delete end
						// add start						
						if ($parentMeta->isModuleEntity() && $parentModule != "Users" && $module != "ContactProduct") {
							//end
