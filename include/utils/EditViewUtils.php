...
function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false)
{
	...
	if (in_array($module, $inventoryModules))
	{
		...
		$query="SELECT
					...
					case when vtiger_products.productid != '' then vtiger_products.qtyinstock else 'NA' end as qtyinstock,
					case when vtiger_products.productid != '' then 'Products' else 'Services' end as entitytype,
					case when vtiger_products.productid != '' then vtiger_products.currency else vtiger_service.currency end as currency,
					vtiger_inventoryproductrel.listprice, vtiger_products.is_subproducts_viewable, 
					vtiger_inventoryproductrel.description AS product_description, vtiger_inventoryproductrel.*,
					vtiger_crmentity.deleted FROM vtiger_inventoryproductrel
					LEFT JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_inventoryproductrel.productid
	...
	elseif ($module == 'Vendors') {
		...
		$query = 'SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.product_no AS productcode, vtiger_products.purchase_cost, vtiger_products.currency,
					vtiger_products.unit_price, vtiger_products.qtyinstock, vtiger_crmentity.deleted, "Products" AS entitytype,
					vtiger_products.is_subproducts_viewable, vtiger_crmentity.description '.$additionalServiceFieldsString.' FROM vtiger_products
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_products.productid
					INNER JOIN vtiger_vendor ON vtiger_vendor.vendorid = vtiger_products.vendor_id
	...
	elseif ($module == 'HelpDesk') {
		/*2020-09-18 Thet Phyo Wai Generate Invoice from Ticket MSCRM Var 1.0 Start*/
		/*$query = 'SELECT vtiger_service.serviceid AS productid, vtiger_service.servicename AS productname, vtiger_service.service_no AS productcode,
					...
					WHERE vtiger_crmentityrel.crmid=? AND vtiger_crmentity.deleted=0 AND vtiger_service.discontinued=1';*/
		$query = "SELECT 
					case when vtiger_products.productid != '' then vtiger_products.productid else vtiger_service.serviceid end as productid,
					case when vtiger_products.productid != '' then vtiger_products.productname else vtiger_service.servicename end as productname,
					case when vtiger_products.productid != '' then vtiger_products.product_no else vtiger_service.service_no end as productcode,
					case when vtiger_products.productid != '' then vtiger_products.unit_price else vtiger_service.unit_price end as unit_price,
					case when vtiger_products.productid != '' then vtiger_products.qtyinstock else 'NA' end as qtyinstock,
					case when vtiger_products.productid != '' then 'Products' else 'Services' end as entitytype,
					vtiger_ticketsproductrel.listprice, vtiger_products.is_subproducts_viewable,vtiger_ticketsproductrel.qty AS quantity, vtiger_ticketsproductrel.usedcurrency AS usedcurrency,
					vtiger_crmentity.deleted
					FROM vtiger_ticketsproductrel
					LEFT JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_ticketsproductrel.productid
					LEFT JOIN vtiger_products ON vtiger_products.productid=vtiger_ticketsproductrel.productid
					LEFT JOIN vtiger_service ON vtiger_service.serviceid=vtiger_ticketsproductrel.productid
					WHERE ticketid=? ORDER BY entitytype";
		/*2020-09-18 Thet Phyo Wai Generate Invoice from Ticket MSCRM Var 1.0 End*/
			$params = array($seid);
	}
	...
	{	
		/*2020-11-09 Pyae Phyo Mon Purchase Cost Currency Change Start*/
		/*$query='SELECT vtiger_service.serviceid AS productid, vtiger_service.servicename AS productname, vtiger_service.service_no AS productcode,
		...
		$query='SELECT vtiger_service.serviceid AS productid, vtiger_service.servicename AS productname, vtiger_service.service_no AS productcode,
					vtiger_service.purchase_cost,vtiger_service.currency, vtiger_service.unit_price AS unit_price, "NA" AS qtyinstock, vtiger_crmentity.deleted,
					"Services" AS entitytype, 1 AS is_subproducts_viewable, vtiger_crmentity.description '.$additionalServiceFieldsString.' FROM vtiger_service
					INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_service.serviceid
					INNER JOIN vtiger_servicecf ON vtiger_service.serviceid = vtiger_servicecf.serviceid
					WHERE vtiger_crmentity.deleted=0 AND vtiger_service.serviceid=?';
		/*2020-11-09 Pyae Phyo Mon Purchase Cost Currency Change End*/
		$params = array($seid);
	}
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		/*2020-09-30 Thet Phyo Wai Ticket Currency Add MSCRM Ver 1.0 Start*/
		if ($module == 'HelpDesk') {
			$usedCurrency = $adb->query_result($result, $i-1, 'usedcurrency');
			$product_Detail[$i]['usedcurrency'.$i] = $usedCurrency;
		}
		/*2020-09-30 Thet Phyo Wai Ticket Currency Add MSCRM Ver 1.0 Start*/
	
		/*2020-11-09 Thet Phyo Wai Purchase Cost Currency Change Start*/
		if ($module == 'Products' || $module == 'Vendors' || $module == 'Services' || in_array($module, $inventoryModules)) {
			$product_Detail[$i]['currency'.$i] = $adb->query_result($result, $i-1, 'currency');
		}
		/*2020-11-09 Thet Phyo Wai Purchase Cost Currency Change End*/
