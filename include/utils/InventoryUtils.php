...
function getPurchaseCostCurrency($productid,$module='Products') {
	global $adb, $log;
	if ($module == 'Services') {
		$sql = "select currency from vtiger_service where serviceid=?";
	} else {
		$sql = "select currency from vtiger_products where productid=?";
	}
	$params = array($productid);
	$res = $adb->pquery($sql, $params);
	$currencyid = $adb->query_result($res, 0, 'currency');
	return $currencyid;
}
/*2020-11-05 Thet Phyo Wai Purchase Cost Currency Change Start*/

/*2021-06-28 Myo Min Kyaw Show correct currency symbol at Contract Price at Customer Product link listing and detail page start*/
function getContractPriceCurrency($customerProductLinkid) {
	global $adb, $log;
	$sql = "select currency from vtiger_contactproduct where contactproduct_id=?";
	$params = array($customerProductLinkid);
	$res = $adb->pquery($sql, $params);
	$currencyid = $adb->query_result($res, 0, 'currency');
	return $currencyid;
}
/*2021-06-28 Myo Min Kyaw Show correct currency symbol at Contract Price at Customer Product link listing and detail page end*/
