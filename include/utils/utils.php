...
function DeleteEntity($module, $return_module, $focus, $record, $return_id)
{
	...
	if ($return_module == 'Accounts' && $module == 'Contacts') {
		global $adb;
		$adb->pquery("UPDATE vtiger_crmentity set is_update = 1 where crmid = ?", array($return_id));
	}
	//2020-11-24 Pyae Phyo Mon update the crmentity is_update field update MSCRM ver1.0 End
...
//2021-07-05 Myo Min Kyaw update the last active fieldd in ticket activity create start
function addLastActiveFieldInTicket($parentRecordId)
{
	$db = PearDatabase::getInstance();
	$pId = intval($parentRecordId);
	$result = $db->pquery("SELECT * From vtiger_seactivityrel where crmid=?", array($pId));
	$noofrows = $db->num_rows($result);
	$t = array();
	for ($i = 0; $i < $noofrows; $i++) {
		$actID = $db->query_result($result, $i, 'activityid');
		$actid = intval($actID);
		$rel =  $db->pquery("SELECT due_date, time_end FROM vtiger_activity where activityid=?", array($actid));
		$noofrowsdfsds = $db->num_rows($rel);
		$end_date = $db->query_result($rel, 0, 'due_date');
		$end_time = $db->query_result($rel, 0, 'time_end');
		$t[$i] = $end_date;
	}
	$mostRecent = 0;
	foreach ($t as $date) {
	...
	$result = date('Y-m-d', $mostRecent);

	$databaseUpdateResult = $db->pquery("UPDATE vtiger_troubletickets SET last_activity = ? WHERE ticketid=?", array($result, $parentRecordId));

	if (!$databaseUpdateResult) {
		global $log;
		$log->debug("Ticket last active field update fail" + date('Y-m-d H:i:s'));
	}
}
//2021-07-05 Myo Min Kyaw update the last active fieldd in ticket activity create end
//2021-07-05 Myo Min Kyaw Contract price amoutn problem start
function getMmkRate()
{
	$db = PearDatabase::getInstance();
	$result = $db->pquery("SELECT conversion_rate FROM vtiger_currency_info WHERE currency_name = ?", array('Myanmar, MMK'));
	$d = $db->query_result($result, 0, 'conversion_rate');
	return $d;
}
//2021-07-05 Myo Min Kyaw Contract price amoutn problem end

function getDollarRate()
{
	$db = PearDatabase::getInstance();
	$result = $db->pquery("SELECT conversion_rate FROM vtiger_currency_info WHERE currency_name = ?", array('USA, Dollars'));
	$d = $db->query_result($result, 0, 'conversion_rate');
	return $d;
}

function getSingRate()
{
	$db = PearDatabase::getInstance();
	$result = $db->pquery("SELECT conversion_rate FROM vtiger_currency_info WHERE currency_name = ?", array('Singapore, Dollars'));
	$d = $db->query_result($result, 0, 'conversion_rate');
	return $d;
}

function getJapanRate()
{
	$db = PearDatabase::getInstance();
	$result = $db->pquery("SELECT conversion_rate FROM vtiger_currency_info WHERE currency_name = ?", array('Japan, Yen'));
	$d = $db->query_result($result, 0, 'conversion_rate');
	return $d;
}
...
function getUserCurrency($id)
{
	...
	$db = PearDatabase::getInstance();
	$result = $db->pquery("SELECT currency_id FROM vtiger_users WHERE id = ?", array($id));
	$d = $db->query_result($result, 0, 'currency_id');
	$result2 = $db->pquery("SELECT currency_code FROM vtiger_currency_info WHERE id = ?", array($d));
	$user_currency_code = $db->query_result($result2, 0, 'currency_code');
