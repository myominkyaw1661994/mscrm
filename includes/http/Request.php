...
class Vtiger_Request {
	...
	protected function validateReferer() {
		...
		if (isset($_SERVER['HTTP_REFERER']) && $user) {//Check for user post authentication.
			global $site_URL;
			 $site_ssl_URL = 'https://www.activebizs.com/MSCRM_Release';
			if ((stripos($_SERVER['HTTP_REFERER'], $site_URL) !== 0) && (stripos($_SERVER['HTTP_REFERER'], $site_ssl_URL) !== 0) && ($this->get('module') != 'Install')) {
