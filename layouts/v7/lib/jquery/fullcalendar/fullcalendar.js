...
Grid.mixin({
	...
	getSegBackgroundColor: function(seg) {
		// 2020/02/11 SuLatt Travel CRM customize for MIA start
		if(seg.event.color == false || seg.event.color == 'undefind'){
			var calendarFeedList = jQuery('#calendarview-feeds > ul.feedslist');
			var li_text =calendarFeedList.find('li');	


			for(var i=0;i<li_text.length;i++) {
				// find input tag
				var id_text = li_text.find('input');

				// get user id from tag attributes
				var userid = id_text[i]['attributes'][6];
				var fieldName = id_text[i]['attributes'][6];

				// get background-color of user list
				var style_str = li_text[i]['attributes'][1].value.toString();
				var position = style_str.indexOf(':')+2;
				style_str = style_str.substr(position);
				position = style_str.indexOf(';');
				style_str = style_str.substr(0, position);

				// set background color by user id
				if(seg.event.fieldName == fieldName.value || userid.value == seg.event.userid) {
					console.log(fieldName.value);
					if(id_text[i]['attributes'][4].value != '') {
						seg.event.color = id_text[i]['attributes'][4].value;
					 	break;
					}
					else {
						seg.event.color = style_str;
						break;
					}
				}
			}		
		}
		// 2020/02/11 SuLatt Travel CRM customize for MIA end
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		classes.unshift('fc-day-grid-event', 'fc-h-event', 'fc-color-event');		 // sulatt 
