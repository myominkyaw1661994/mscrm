...
Vtiger_Edit_Js("Calendar_Edit_Js",{
	...
	registerPotentialLead : function(container){
		if(jQuery('input[name="sourceModule"]',container).length >0 && jQuery('input[name="sourceModule"]',container).val() =='Potentials'){
			var self = this;
			var leadId = jQuery('input[id="leadId"]',container).val();
			var LeadName = jQuery('input[id="leadName"]',container).val();
			var data = {
							'name' : LeadName,
							'id' : leadId
						}
			var leadElem = self.getParentElement(jQuery('input[name="lead_id"]',container));
			self.setReferenceFieldValue(leadElem, data);
		}
	},
	...
	registerBasicEvents : function(container) {
		...
		/*2020-10-14 Thet Phyo Wai Update Opportunity Activity MSCRM Ver 1.0 Start*/
        this.registerPotentialLead(container);  
        /*2020-10-14 Thet Phyo Wai Update Opportunity Activity MSCRM Ver 1.0 End*/
	}
});
