...
{strip}
	...
	{else}
		<div class="relatedContainer">
			...
			{*2020-12-11 Thet Phyo Wai Ticket Product Next Page Cann't go MSCRM Start*}
			<input type='hidden' value="{$PAGING->get('page')}" id='pageNumber'>
			<input type="hidden" value="{$PAGING->isNextPageExists()}" id="nextPageExist"/>
			{*2020-12-11 Thet Phyo Wai Ticket Product Next Page Cann't go MSCRM End*}
							...
							<tr class="searchRow">
								...
								{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
									<th>
										...
										{if $HEADER_FIELD->get('column') eq 'time_start' or $HEADER_FIELD->get('column') eq 'time_end' or $HEADER_FIELD->getFieldDataType() eq 'reference' or $HEADER_FIELD->get('name') eq 'qty' or $HEADER_FIELD->get('name') eq 'listprice'}
									...
									<span class="actionImages">
										{if (!empty($RELATED_HEADERS['listprice']) || !empty($RELATED_HEADERS['unit_price']))}
											{if !empty($RELATED_HEADERS['listprice'])}
												...
												{assign var="QTY" value=$RELATED_RECORD->get('qty')}
											{/if}
										{/if}
										<a href="javascript:void(0);" data-url="index.php?module=HelpDesk&view=ListPriceUpdate&record={$PARENT_RECORD->getId()}&relid={$RELATED_RECORD->getId()}&currentPrice={$LISTPRICE}&currentQty={$QTY}"
											class="editListPrice cursorPointer" data-related-recordid='{$RELATED_RECORD->getId()}' data-qty='{$QTY}' data-list-price={$LISTPRICE}><i title="{vtranslate('LBL_EDIT', $MODULE)}" class="fa fa-pencil"></i></a> &nbsp;&nbsp;
										<a class="relationDelete"><i title="{vtranslate('LBL_UNLINK', $MODULE)}" class="vicon-linkopen"></i></a>
									</span>
								</td>
								{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
									{assign var=RELATED_HEADERNAME value=$HEADER_FIELD->get('name')}
									<td nowrap class="{$WIDTHTYPE} tt">
										{if $RELATED_HEADERNAME == 'listprice'}
										{assign var=CURRENCY_SYMBOL value=getCurrencySymbolandCRate($RELATED_RECORD->get('usedcurrency'))}
											{assign var="LISTPRICE" value=$RELATED_RECORD->get($HEADER_FIELD->get('name'))}
											{CurrencyField::appendCurrencySymbol($LISTPRICE, $CURRENCY_SYMBOL['symbol'])}
										{else if $RELATED_HEADERNAME == 'qty'}
										...
										{elseif $HEADER_FIELD->get('uitype') eq '71' or $HEADER_FIELD->get('uitype') eq '72'}
		                                    ...
		                                    {if $HEADER_FIELD->get('uitype') eq '71' and $RELATED_HEADERNAME eq 'purchase_cost'}
