/*2020-09-17 Thet Phyo Wai Create Related Ticket MSCRM var 1.0 Add*/
/*+***********************************************************************************
 ...
 * All Rights Reserved.
 *************************************************************************************/
Vtiger_Popup_Js("Ticket_Products_Popup_Js",{
    registered : false
},{
                ...
                submitHandler: function(form) {
                    ...
                    for(var data in selectedRecords){
                        if(typeof selectedRecords[data] == "object"){
                            ...
                            var rowQty = row.find('input[name=qty]');
                            var qty = rowQty.val();
                            var rowListPrice = row.find('input[name=unit_price]');
                            var listPrice = rowListPrice.val();
                            selectedRecordDetails[id] = {'id' : id,'price' : listPrice,'qty' : qty};
                        }
                    }
	...
	selectAllHandler : function(e){
		...
		if(isMainCheckBoxChecked) {
			jQuery('input.entryCheckBox', tableElement).closest('tr').find('input[name="qty"]').removeClass('hide');
			jQuery('input.entryCheckBox', tableElement).closest('tr').find('input[name="unit_price"]').removeClass('hide');
		}else {
			jQuery('input.entryCheckBox', tableElement).closest('tr').find('input[name="qty"]').addClass('hide');
	...
	sortHandler : function(headerElement){
		var thisInstance = this;
		/*2020-10-22 Thet Phyo Wai Ticket Related Product/Service Sorting MSCRM Ver 1.0 Start*/
		var aDeferred = jQuery.Deferred();
		/*2020-10-22 Thet Phyo Wai Ticket Related Product/Service Sorting MSCRM Ver 1.0 End*/
	...
	registerEvents : function(){
        if(!Ticket_Products_Popup_Js.registered) {
            var thisInstance = this;
            Ticket_Products_Popup_Js.registered = true;
            this._super();
