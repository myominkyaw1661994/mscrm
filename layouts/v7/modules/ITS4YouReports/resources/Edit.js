 ...
 */
    ...
    validateCurrentStep: function (step) {
         ...
         * */
            ...
            if (jQuery('#isReportScheduled').is(':checked')) {
                ...
                if ('checked' === jQuery('#scheduledReportFormat_csv').attr('checked')
                    && ('' !== jQuery('#csv_name').val() || '' !== jQuery('#csv_path').val())) {
                    vtUtils.hideValidationMessage(jQuery('#scheduledReportFormat_pdf'));
                    vtUtils.hideValidationMessage(jQuery('#scheduledReportFormat_xls'));
                    vtUtils.hideValidationMessage(jQuery('#scheduledReportFormat_csv'));
                    vtUtils.hideValidationMessage(jQuery('#selectedRecipients'));
                    vtUtils.hideValidationMessage(jQuery('#generate_other'));

                    if ('' === jQuery('#csv_name').val()) {
                        vtUtils.showValidationMessage(jQuery('#csv_name'), app.vtranslate('JS_REQUIRED_FIELD'));
                        returnValidate = false;
                    }
                    ...
                    if (!jQuery('#scheduledReportFormat_pdf').is(':checked')
                        && !jQuery('#scheduledReportFormat_xls').is(':checked')
                        && !jQuery('#scheduledReportFormat_csv').is(':checked')) {
                        vtUtils.showValidationMessage(jQuery('#scheduledReportFormat_pdf'), app.vtranslate('JS_REQUIRED_FIELD'));
                        vtUtils.showValidationMessage(jQuery('#scheduledReportFormat_xls'), app.vtranslate('JS_REQUIRED_FIELD'));
                        vtUtils.showValidationMessage(jQuery('#scheduledReportFormat_csv'), app.vtranslate('JS_REQUIRED_FIELD'));
                        returnValidate = false;
                    } else {
                        vtUtils.hideValidationMessage(jQuery('#scheduledReportFormat_pdf'));
                        vtUtils.hideValidationMessage(jQuery('#scheduledReportFormat_xls'));
    ...
    validateStepsOfReport: function (step) {
        ...
        if (10 === parseInt(currentStep)
            && 'checked' === jQuery('#scheduledReportFormat_csv').attr('checked')
            && 'undefined' === typeof window.pathValidationResult) {
            if ('' !== jQuery('#csv_name').val() || '' !== jQuery('#csv_path').val()) {
                let actionParams = {
                    "type": "POST",
                    "url": 'index.php?action=IndexAjax&mode=checkCsvPath&module=ITS4YouReports',
                    "dataType": "html",
                    "data": {
                        "csv_path": jQuery('#csv_path').val(),
                    }
                };

                if (window.pathValidationInProgress) {
                    thisInstance.validateStepsOfReport(step);
                }

                app.helper.showProgress();
                window.pathValidationInProgress = true;

                app.request.post(actionParams).then(
                    function (err, data) {
                        app.helper.hideProgress();
                        window.pathValidationInProgress = false;

                        if (err === null) {

                            if (true !== data.success) {
                                vtUtils.showValidationMessage(jQuery('#csv_path'), data.message);
                                window.pathValidationResult = false;
                            } else {
                                window.pathValidationResult = true;
                                if ('undefined' !== typeof step) {
                                    thisInstance.changeSteps4U(step);
                                } else {
                                    thisInstance.changeSteps(window.savetype);
                                }
                            }
                        } else {
    ...
    changeSteps4U: function (step, stype) {
		...
		if (!gotoStep) {
            ...
            */
            ...
            let dashboard_chart = 0;
            if (true === jQuery('#dashboard_chart').is(":checked")) {
                dashboard_chart = 1;
            }
            postData["dashboard_chart"] = dashboard_chart;
            let dashboard_table = 0;
            if (true === jQuery('#dashboard_table').is(":checked")) {
                dashboard_table = 1;
            }
            postData["dashboard_table"] = dashboard_table;
