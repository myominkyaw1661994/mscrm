...
function reports4you_updatefOptions(sel, opSelName, selectedVal) {
    jQuery(document).ready(function() {
        ...
        typeofdata['DT'] = ['e', 'n', 'l', 'g', 'm', 'h', 'b', 'a', 'isn', 'isnn'];
...
function changeSteps4U(step) {
    ...
    if (!gotoStep) {
        ...
        newurl += '&csv_path=' + $('input[name="csv_path"]').val();
        newurl += '&csv_name=' + $('input[name="csv_name"]').val();
    ...
    app.request.post(actionParams).then(
        function(err,dataResult) {
            if(err === null){
                ...
                if(-1 < jQuery.inArray(fop_type_val,JSON.parse(jQuery('#fld_date_options').val()))){
                    var fieldValue = getFieldLabelExpression(jQuery('#fvalhdn' + row_i).val());
                ...
                } else if ('isn' === fop_type_val || 'isnn' === fop_type_val) {
                    document.getElementById("node3span" + row_i + "_djx").innerHTML = dataResult;
                    document.getElementById("node3span" + row_i + "_st").style.display = "none";
                    document.getElementById("node3span" + row_i + "_ajx").style.display = "none";
                    document.getElementById("fval" + row_i).value = "";
    ...
    if (groupObj) {
        ...
        if (document.getElementById("date_options_json")) {
            ...
            if (date_options_json) {
                var date_options = JSON.parse(date_options_json);

                if (0 < date_options.indexOf(selected_option)) {
...
jQuery(document).ready(function() {
    ...
    jQuery('#scheduledReportFormat_csv').on('click', function (e) {
        let csvPathElm = jQuery('#csv_path');
        let csvNameElm = jQuery('#csv_name');

        if ($(this).prop('checked')) {
            csvPathElm.removeAttr('disabled');
            csvNameElm.removeAttr('disabled');
        } else {
            csvPathElm.val('');
            csvPathElm.attr('disabled', 'disabled');
            csvNameElm.val('');
            csvNameElm.attr('disabled', 'disabled');
        }
    });

    jQuery('#csv_name').on('blur', function(){
        let thisElm = $(this);
        if ('' !== thisElm.val()) {
            if ('.csv' !== thisElm.val().substr(-4)) {
                thisElm.val(thisElm.val() + '.csv');
            }
        }
    });
