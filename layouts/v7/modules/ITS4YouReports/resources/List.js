...
Vtiger_List_Js("ITS4YouReports_List_Js",{
	...
	registerDownloadButton: function () {
		let editLicense = jQuery('#editLicense');
		editLicense.find('#download_button').on('click', function () {
			var params = {
				'module': 'ITS4YouReports',
				'action': 'IndexAjax',
			...
			app.request.post({'data': params}).then(function (error, data) {
				...
				if (data.success) {
					...
					editLicense.find('#download_button').addClass('hide');
					editLicense.find('#finish_button').removeClass('hide');
				} else {
					app.hideModalWindow();
					var params = {
						title: data.message,
