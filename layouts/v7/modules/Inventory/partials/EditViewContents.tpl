                 ...
                 <table class="table table-borderless {if $BLOCK_LABEL eq 'LBL_ADDRESS_INFORMATION'} addressBlock{/if}">
                    ...
                    {/if}
                     ...
                     {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
                        {* 2020/09/17 Pyae Phyo Mon Permission PDF Output MSCRM Ver 1.0 start *}
                        {if $MODULE eq 'PurchaseOrder' and $FIELD_NAME eq "approve" and $IS_ADMIN eq false}
                            {continue}
                        {/if}
                        {* 2020/09/17 Pyae Phyo Mon Permission of PDF Output MSCRM Ver 1.0 end *}

                        {*2020-09-23 Thet Phyo Wai Customize Invoice Module MSCRM Ver 1.0 Start*}
                        {if $MODULE eq 'Invoice'}
                        {if ($SOURCE_MODULE neq 'HelpDesk' and $FIELD_MODEL->get('fieldvalue') eq '') and ($FIELD_NAME eq 'ticket_number' or $FIELD_NAME eq 'product_id' or $FIELD_NAME eq 'machine_serial_no' ) }
                        {continue}
                        {/if}
                        {/if}
