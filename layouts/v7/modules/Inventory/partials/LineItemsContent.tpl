...
{strip}
	...
	{if $MODULE eq 'PurchaseOrder'}
		...
		{*2020-11-17 Thet Phyo Wai Duplicate PO List Price Empty MSCRM Start*}
		{if $purchaseCost eq 0}
		{assign var="purchaseCost" value="{if $data.$listPrice}{((float)$data.$listPrice) / ((float)$data.$qty * {$RECORD_CURRENCY_RATE})}{else}0{/if}"}
		{/if}
		{*2020-11-17 Thet Phyo Wai Duplicate PO List Price Empty MSCRM End*}
	...
	{if $LIST_PRICE_EDITABLE}
		<td>
			<div>
				{*2020-10-22 Thet Phyo Wai Ticket Price Calculate MSCRM Ver 1.0 Start*}
				...
				<input id="{$listPrice}" name="{$listPrice}" value="{if !empty($data.$listPrice)}{$data.$listPrice}{else}0{/if}" type="text"
					   data-rule-required=true data-rule-positive=true class="listPrice smallInputBox inputElement" data-is-price-changed="{if ($SOURCE_MODULE eq 'HelpDesk' or $RECORD_ID) && $row_no neq 0}true{else}false{/if}" list-info='{if isset($data.$listPrice)}{Zend_Json::encode($listPriceValues)}{/if}' data-base-currency-id="{getProductBaseCurrency($productId, {$entityType})}" />
				{*2020-10-22 Thet Phyo Wai Ticket Price Calculate MSCRM Ver 1.0 End*}
				&nbsp;
