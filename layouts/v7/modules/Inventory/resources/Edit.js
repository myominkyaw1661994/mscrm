...
Vtiger_Edit_Js("Inventory_Edit_Js", {
	...
	// 2020/09/11 Pyae Phyo Mon Vendor information Show in PO MSCRM Ver 1.0 Start
	//Will have the mapping of information fields based on the modules
	informationFieldsMapping : {
                                'Vendors' : {
                                    'attention_name': 'attention_name',
                                    'position': 'position',
                                    'vendor_phone':'phone',
                                    'vendor_email':'email',
                                    'vendor_incoterms': 'incoterms',
                                    'payment_term':'payment_term',
                                    'address':'street',
                                    'estimate_arrival_date':'lead_time'
                                } ,
                            },
	// 2020/09/11 Pyae Phyo Mon Vendor information Show in PO MSCRM Ver 1.0 End
	...
	calculateDiscountForLineItem : function(lineItemRow) {
		...
		if(discountType == Inventory_Edit_Js.percentageDiscountType) {
			...
			/*2020-10-23 Thet Phyo Wai limit Decimal Discount MSCRM Ver 1.0 Start*/
			discountValueDecimal = parseFloat(discountValue).toFixed(this.numOfCurrencyDecimals);
			var lineItemDiscount = '('+discountValueDecimal+')';
			/*2020-10-23 Thet Phyo Wai limit Decimal Discount MSCRM Ver 1.0 Start*/
		}
    ...
    calculateFinalDiscount : function() {
		...
		if(discountType == Inventory_Edit_Js.percentageDiscountType){
			...
			/*2020-10-23 Thet Phyo Wai limit Decimal Discount MSCRM Ver 1.0 Start*/
			discountValueDecimal = parseFloat(discountValue).toFixed(numberOfDecimal);
			var overallDiscount = '('+discountValueDecimal+')';
			/*2020-10-23 Thet Phyo Wai limit Decimal Discount MSCRM Ver 1.0 End*/
            if(this.prevSelectedCurrencyConversionRate){
                ...
                discountRow.find('.discountVal').val(parseFloat(discountValue).toFixed(numberOfDecimal));
            	/*2020-10-23 Thet Phyo Wai limit Decimal Discount MSCRM Ver 1.0 End*/
            }
            rowAmountField.removeClass('hide').focus();
		}
...
//		this.calculateGrandTotal();
    ...
    AdjustmentShippingResultCalculation: function(conversionRate){
		...
		jQuery(adjustmentElement).val(newAdjustment.toFixed(this.numOfCurrencyDecimals));
		/*2020-10-23 Thet Phyo Wai limit Decimal Discount MSCRM Ver 1.0 End*/
    ...
    mapResultsToFields: function(parentRow,responseData){
		...
		for(var id in responseData){
			...
			/*2020-10-20 Thet Phyo Wai Decimal Limit Selected Item MSCRM Ver 1.0 Start*/
			//jQuery('input.listPrice',parentRow).val(unitPrice);
			var numberofDecimal = app.getNumberOfDecimals();
			jQuery('input.listPrice',parentRow).val(unitPrice.toFixed(numberofDecimal));
			/*2020-10-20 Thet Phyo Wai Decimal Limit Selected Item MSCRM Ver 1.0 End*/
	...
	pricebooksPopupHandler : function(popupImageElement){
		...
		/*2020-08-28 Thet Phyo Wai Price Book List Wrong in all Related Module Start*/
		...
		params.src_module = lineItemProductOrServiceElement.find('span.lineItemPopup').data('moduleName');
		params.src_field = lineItemProductOrServiceElement.find('span.lineItemPopup').data('fieldName');
		/*2020-08-28 Thet Phyo Wai Price Book List Wrong in all Related Module End*/
	 ...
	 */
     ...
     registerCurrencyChangeEvent : function() {
		...
		this.currencyElement.change(function(e){
			...
			/*2020-12-03 Thet Phyo Wai Change Currency Readyonly MSCRM Var 1.0 Start*/
			var modulename = jQuery('[name="module"]').val();
			var selectedCurrencyId = jQuery('#selectedCurrencyId').val();
			if(modulename == "PurchaseOrder" || modulename == "Invoice" ){
				if(currencyId == selectedCurrencyId){
					jQuery('#received').attr('readonly', false);
					jQuery('#paid').attr('readonly', false);
					element.closest('div').find('.message-text').addClass("hide");
				}else{
					jQuery('#received').attr('readonly', true);
					jQuery('#paid').attr('readonly', true);
					element.closest('div').find('.message-text').removeClass("hide");
				}
			}
			/*2020-12-03 Thet Phyo Wai Change Currency Readyonly MSCRM Var 1.0 End*/
			...
			self.lineItemsHolder.find('tr.'+self.lineItemDetectingClass).each(function(index,domElement){
                ...
                /*2020-11-04 Thet Phyo Wai Check Price with More Currency MSCRM Ver 1.0 Start*/
                /*if (isPriceChanged == false) {*/
                var beforeCurrencyId = jQuery('#prev_selected_currency_id').val();
                if (isPriceChanged == false || parseFloat(listPriceValues[beforeCurrencyId]) == listPriceVal) {
                	listPriceElement.data('isPriceChanged', false);
                /*2020-11-04 Thet Phyo Wai Check Price with More Currency MSCRM Ver 1.0 End*/
...
//			};
	...
	referenceSelectionEventHandler : function(data,container){
		...
		if (data['selectedName']) {
			// 2020/09/11 Pyae Phyo Mon Vendor information Show in PO MSCRM Ver 1.0 Start
			var modulename = jQuery('[name="module"]').val();
			if(modulename='PurchaseOrder' && sourceModule =='Vendors')
			{
				self.copyInformationDetails(data, container);
			}
			// 2020/09/11 Pyae Phyo Mon Vendor information Show in PO MSCRM Ver 1.0 End
	...
	// 2020/09/11 Pyae Phyo Mon Vendor information Show in PO MSCRM Ver 1.0 Start
	copyInformationDetails : function(data,container) {
        var self = this;
        var sourceModule = data['source_module'];
        var noAddress = true;
        var errorMsg;
        ...
        this.getRecordDetails(data).then(
            function(data){
                var response = data;
                self.vendorDetail(self.informationFieldsMapping[sourceModule], response['data'], container);
            },
            function(error, err){
    ...
    vendorDetail : function(informationDetails, result, container) {
        for(var key in informationDetails) {
        	if(key =="estimate_arrival_date")
        	{
        		var dateFormat = app.getDateFormat();
        		var today = new Date(); 
        		var time = parseInt(result[informationDetails[key]]);
				today.setDate(today.getDate() + time);
				var estimateDate=moment(today).format(dateFormat.toUpperCase());
        		container.find('[name="'+key+'"]').val(estimateDate);
        	}
        	else
        	{
        		container.find('[name="'+key+'"]').val(result[informationDetails[key]]);
        	}				
            container.find('[name="'+key+'"]').trigger('change');
        }
    },
    ...
    /*2020-10-26 Thet Phyo Wai Get Vendor Data in PO MSCRM Ver 1.0 Start*/
    registerVendorDataEvent : function() {
        var container = this.getForm();
        var recordId = container.find('[name="record"]').val();
        if(this.getModuleName() == "PurchaseOrder" && ( recordId == undefined || recordId == '' || recordId == 0)){
        	var vendorId = container.find('[name="vendor_id"]');
        	if(!(vendorId.val() == undefined || vendorId.val() == '' || vendorId.val() == 0)){
	        	var params = {
		            'source_module' : 'Vendors',
		            'record' : vendorId.val()
		        };
		        this.copyInformationDetails(params, container);
		    }
        }
    },
    /*2020-10-26 Thet Phyo Wai Get Vendor Data in PO MSCRM Ver 1.0 End*/
