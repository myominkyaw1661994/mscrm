 ...
 * All Rights Reserved.
 ************************************************************************************}
{* modules/Products/views/TicketProductPopupAjax.php *}
...
{strip}
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="popupEntriesDiv relatedContents contents-bottomscroll TicketProductPopup">
                ...
                <input type="hidden" value="Ticket_Products_Popup_Js" id="popUpClassName"/>
                {*<input type="hidden" value="{Vtiger_Util_Helper::toSafeHTML(Zend_JSON::encode($SEARCH_DETAILS))}" id="currentSearchParams" />*}
                    ...
                    <table class="listview-table table-bordered listViewEntriesTable">
                        <thead>
                            <tr class="listViewHeaders">
                                ...
                                {/foreach}
                                <th class="listViewHeaderValues noSorting {$WIDTHTYPE}">{vtranslate('LBL_QTY',$MODULE)}</th>
                        ...
                        {foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=popupListView}
                            ...
                            <tr class="listViewEntries" data-id="{$LISTVIEW_ENTRY->getId()}" data-name='{$LISTVIEW_ENTRY->getName()}'
                                ...
                                <td class="listViewEntryValue textOverflowEllipsis {$WIDTHTYPE}" title="{$RECORD_DATA[$LISTVIEW_HEADERNAME]}">
                                    {if $LISTVIEW_HEADER->get('uitype') eq '72' || $LISTVIEW_HEADER->get('uitype') eq '71'}
                                        ...
                                        {if $LISTVIEW_HEADER->get('uitype') eq '71'}
                                            ...
                                            {if $LISTVIEW_HEADERNAME eq 'purchase_cost'}
                                                {assign var=CURRENCY_SYMBOL value=$LISTVIEW_ENTRY->get('currencySymbol2')}
                                            {/if}
                                ...
                                <td class="listViewEntryValue {$WIDTHTYPE}">
                                    <input type="text" value="{if $EDITED_VALUE}{$EDITED_VALUE["qty"]}{else}1{/if}" name="qty" class="{if !$EDITED_VALUE} hide {/if} inputElement zeroPaddingAndMargin" data-rule-required="true" data-rule-currency="true"/>
                                </td>
                                <td class="listViewEntryValue {$WIDTHTYPE}">
