		...
		<div class="col-sm-6 col-xs-6 portal-related-information">
				...
				<div class="portal-record-privilege">
					{if $RELATED_MODULES[$MODULE]}
						{foreach from=$RELATED_MODULES[$MODULE] key=KEY item=VALUE}
						  {*2020-01-14 Thet Phyo Wai Change Export Location to Record Permission MSCRM Start*}
						  {if $VALUE['name'] neq 'Export'}
						  {*2020-01-14 Thet Phyo Wai Change Export Location to Record Permission MSCRM End*}
			...
			{if $MODULE eq 'HelpDesk' OR $MODULE eq 'Assets'}
				...
				<div class="portal-record-privilege" id="recordPrivilege_{$MODULE}">
					...
					{*2020-01-14 Thet Phyo Wai Change Export Location to Record Permission MSCRM Start*}
					{if $MODULE eq 'HelpDesk' and $EXPORT_VALUE}
						{$VALUE = $EXPORT_VALUE}
						<div class="checkbox label-checkbox">
								<label><input class="relmoduleinfo_{$MODULE}" data-relmodule ="{$VALUE['name']}" type="checkbox" name="{$VALUE['name']}" id="{$VALUE['name']}" value="{$VALUE['value']}" {if $VALUE['value']}checked{/if}/> {vtranslate($VALUE['name'],$QUALIFIED_MODULE)}</label>
							<br>
						</div>
					{/if}
					{*2020-01-14 Thet Phyo Wai Change Export Location to Record Permission MSCRM End*}
