					...
					{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
						...
						<td class="listViewEntryValue" data-name="{$LISTVIEW_HEADER->get('name')}" title="{$LISTVIEW_ENTRY->getTitle($LISTVIEW_HEADER)}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}">
								...
								<span class="value">
									...
									{else}
										...
										{else if $LISTVIEW_HEADER->get('uitype') eq '71'}
											...
											{if $LISTVIEW_HEADERNAME eq 'purchase_cost' and ($MODULE eq 'Products' or $MODULE eq 'Services') }
											{$LISTVIEW_ENTRY->get('currencySymbol2')}{$LISTVIEW_ENTRY_VALUE}
											{else}
											{*2020-11-05 Thet Phyo Wai Purchase Cost Currency Change Start*}
							...
							{if $LISTVIEW_HEADER->isEditable() eq 'true' && $LISTVIEW_HEADER->isAjaxEditable() eq 'true'}
								{*2020-11-24 Thet Phyo Wai Purchase Cost Currency Change Start*}
								{if !($LISTVIEW_HEADERNAME eq 'purchase_cost' and ($MODULE eq 'Products' or $MODULE eq 'Services')) and !($MODULE eq HelpDesk and ($LISTVIEW_HEADERNAME eq "parent_id" or $LISTVIEW_HEADERNAME eq "contact_id" or $LISTVIEW_HEADERNAME eq "product_id" or $LISTVIEW_HEADERNAME eq "ticketstatus" or $LISTVIEW_HEADERNAME eq "machine_serial_no" or $LISTVIEW_HEADERNAME eq "invoice_pattern" or $LISTVIEW_HEADERNAME eq "engineer_name" or $LISTVIEW_HEADERNAME eq "customer_name")) 
									and !($LISTVIEW_HEADERNAME eq 'transfer_to_wd' and ($MODULE eq 'Products' or $MODULE eq 'Vendors'))
									and !($LISTVIEW_HEADERNAME eq 'account_id' and $MODULE eq 'Contacts')
								}
								{*2020-11-24 Thet Phyo Wai Purchase Cost Currency Change End*}
