	...
	<div class='related-tabs row'>
		<ul class="nav nav-tabs">
			...
			{for $i = 0 to $COUNT1-1}
				...
				{* 23-11-2020 Myo Min Kyaw Hide the product at the Contact and Oragnization Moduel MSCRM Ver 1.0  start*}
				{if ($MODULE_NAME eq "Contacts" && $RELATEDMODULENAME eq "Products") || ($MODULE_NAME eq "Accounts" && $RELATEDMODULENAME eq "Products")}
					{continue}	
				{else}
				{* 23-11-2020 Myo Min Kyaw Hide the product at the Contact and Oragnization Moduel MSCRM Ver 1.0  end*}	
				...
				<li class="dropdown related-tab-more-element">
					...
					<ul class="dropdown-menu pull-right" id="relatedmenuList">
						{for $j = $COUNT1 to $COUNT-1}
							...
							{assign var=RELATEDFIELDNAME value=$RELATED_LINK->get('linkFieldName')}
							{assign var="DETAILVIEWRELATEDLINKLBL" value= vtranslate($RELATED_LINK->getLabel(),$RELATEDMODULENAME)}
							{* 2020-10-20 Hide the PBX Manager in relative moduel mscrm ver 1.0 start *}						
							{if $RELATEDMODULENAME eq "PBXManager"}{continue}{/if}
							{*  2020-10-20 Hide the PBX Manager in relative moduel mscrm ver 1.0 end *}
