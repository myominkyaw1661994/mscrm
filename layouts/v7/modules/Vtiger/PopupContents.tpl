        ...
        <div class="popupEntriesDiv relatedContents">
            ...
            <div class="popupEntriesTableContainer {if $MODULE eq 'EmailTemplates'} emailTemplatesPopupTableContainer{/if}">
                ...
                {foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=popupListView}
                    ...
                    {*<tr class="listViewEntries" data-id="{$LISTVIEW_ENTRY->getId()}" {if $MODULE eq 'EmailTemplates'} data-name="{$RECORD_DATA['subject']}" data-info="{$LISTVIEW_ENTRY->get('body')}" {else}  {if $MODULE eq 'ContactProduct'} data-name="{$LISTVIEW_ENTRY->get('machine_serial_no')}" {else} data-name="{$LISTVIEW_ENTRY->getName()}" {/if} data-info='{Vtiger_Util_Helper::toSafeHTML(ZEND_JSON::encode($LISTVIEW_ENTRY->getRawData()))}' {/if}
                    {if $GETURL neq ''} data-url='{$LISTVIEW_ENTRY->$GETURL()}' {/if}  id="{$MODULE}_popUpListView_row_{$smarty.foreach.popupListView.index+1}">*}
                    ...
                    <td class="listViewEntryValue value textOverflowEllipsis {$WIDTHTYPE}" title="{$RECORD_DATA[$LISTVIEW_HEADERNAME]}">
                        ...
                        {else if $LISTVIEW_HEADER->get('uitype') eq '71' and ($MODULE eq 'Products' or $MODULE eq 'Services') and $LISTVIEW_HEADERNAME eq 'purchase_cost'}
                            {$LISTVIEW_ENTRY->get('currencySymbol2')}{$LISTVIEW_ENTRY->get($LISTVIEW_HEADERNAME)}
                        {*2020-11-05 Thet Phyo Wai Purchase Cost Currency Change Start*}
