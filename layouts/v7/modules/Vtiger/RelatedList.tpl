...
{strip}
	...
	<div class="relatedContainer">
		...
		{*2020-10-13 Thet Phyo Wai Remove Bundle Product MSCRM Ver 1.0 Start*}
        {*
		...
		{/if}
		*}
        {*2020-10-13 Thet Phyo Wai Remove Bundle Product MSCRM Ver 1.0 End*}
					...
					<thead>
						<tr class="listViewHeaders">
							...
							{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
								...
								{* 2020-10-20 Myo Min Kyaw hide the field in relative moduel mscrm Ver1.0 start *}
								{if $HEADER_FIELD->get('column') eq 'time_start' or $HEADER_FIELD->get('column') eq 'time_end' or $HEADER_FIELD->get('column') eq 'firstname' or $HEADER_FIELD->get('column') eq ''}
								{* 2020-10-20 Myo Min Kyaw hide the field in relative moduel mscrm Ver1.0 end*}
									<th class="nowrap" style="width:15px">
								{else}
									<th class="nowrap">
									{if $HEADER_FIELD->get('column') eq "access_count" or $HEADER_FIELD->get('column') eq "idlists" or $HEADER_FIELD->get('column') eq "firstname" or $HEADER_FIELD->get('column') eq ""}
						...
						<tr class="searchRow">
							...
							{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
								<th>
									{* 2020-10-20 Myo Min Kyaw hide the field in relative moduel mscrm Ver1.0 start*}
									{if $HEADER_FIELD->get('column') eq 'time_start' or $HEADER_FIELD->get('column') eq 'time_end' or $HEADER_FIELD->getFieldDataType() eq 'reference' or $HEADER_FIELD->get('column') eq 'firstname'}
									{* 2020-10-20 Myo Min Kyaw hide the field in relative moduel mscrm Ver1.0 end*}
									{else}
							...
							<td class="related-list-actions">
								<span class="actionImages">&nbsp;&nbsp;&nbsp;
									...
									{*2021-08-27 Thet Phyo Wai Create CSC Product Module End*}
										...
										{* 2020/10/01 Myo Min Kyaw Change edit qty to Edit product bundle mscrm ver 1.1 start *}		
										...
										{else if $MODULE eq '' && $RELATED_MODULE_NAME eq 'Products' && $TAB_LABEL === 'Product Bundles' && $RELATED_LIST_LINKS && $PARENT_RECORD->isBundle()}

										{* 2020/10/01 Myo Min Kyaw Change edit qty to Edit product bundle mscrm ver 1.1 end *}
									...
									<span class="value textOverflowEllipsis">
										{if $RELATED_MODULE_NAME eq 'Documents' && $RELATED_HEADERNAME eq 'document_source'}
											...
											{elseif $HEADER_FIELD->get('uitype') eq '71' or $HEADER_FIELD->get('uitype') eq '72'}
												...
												{*2020-11-06 Thet Phyo Wai Purchase Cost Currency Change Start*}
			                                    {if $HEADER_FIELD->get('uitype') eq '71' and  ($RELATED_MODULE_NAME eq 'Products' or $RELATED_MODULE_NAME eq 'Services') and $RELATED_HEADERNAME eq 'purchase_cost'}
			                                        {assign var=CURRENCY_SYMBOL_RATE value=getCurrencySymbolandCRate($RELATED_RECORD->get('currency'))}
			                                        {assign var=CURRENCY_SYMBOL value=$CURRENCY_SYMBOL_RATE['symbol']}
			                                        {assign var=CURRENCY_VALUE value=CurrencyField::convertToUserFormat($RELATED_RECORD->get($RELATED_HEADERNAME), null, true)}
			                                    {/if}
			                                    {*2020-11-06 Thet Phyo Wai Purchase Cost Currency Change Start*}
