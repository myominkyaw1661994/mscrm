	...
	deleteActivityRelation : function(customParams) {
		...
		app.request.post({data: params}).then(function(err, data) {
			...
			/*2020-10-01 Thet Phyo Wai Completed Activity MSCRM Ver 1.0 End*/
				...
				/*2020-10-01 Thet Phyo Wai Completed Activity MSCRM Ver 1.0 Start*/
			/*}*/
				params['mode'] = 'getCompletedActivities';

				app.request.get({data: params}).then(function(err, result) {
					jQuery('#completedActivities').html(result);
					Vtiger_Detail_Js.getInstance().registerEventForActivityWidget();
				});
				/*2020-10-01 Thet Phyo Wai Completed Activity MSCRM Ver 1.0 End*/
		...
		if(jQuery('.editElement',editElement).length === 0){
			...
			if(self.getOverlayDetailMode() == true){
				fieldInfo = related_uimeta.field.get(fieldName);
				 ...
				 /*Myo Min Kyaw how correct currency symbol at Contract Price at Customer Product link listing and detail page start*/
				 if(fieldType == "currency" && fieldName == "contract_price"){
				 	var contact_price_innerText = currentTdElement[0].innerText;
				 	var take_symbol = contact_price_innerText.split(/\s{1}/);
				 	fieldInfo.currency_symbol = take_symbol[0];
				 }
				 /*Myo Min Kyaw how correct currency symbol at Contract Price at Customer Product link listing and detail page start*/
			}
	...
	registerEventForActivityWidget : function(){
		...
		jQuery('.createActivity').on('click', function(e){
			...
			app.event.on('post.QuickCreateForm.save',function(event,data){
				...
				app.request.post({"data":params}).then(
					function(err,data) {
						var activitiesWidget = jQuery('#completedActivities');
						activitiesWidget.html(data);
						vtUtils.applyFieldElementsView(activitiesWidget);
						thisInstance.registerEventForActivityWidget();
					}
