	...
	registerQuickCreateEvent : function (){
		...
		jQuery("#quickCreateModules").on("click",".quickCreateModule",function(e,params){
			...
			thisInstance.getQuickCreateForm(quickCreateUrl,quickCreateModuleName,params).then(function(data){
				...
				/*2020/11/10 Pyae Phyo Mon Add currency in Quick Create Ver 1.0 Add Start*/
				if(moduleName == "ContactProduct" || moduleName == "Products" || moduleName == "Services" ){

	            if(moduleName == "ContactProduct"){
	                var priceSymbol = form.find('input[name="contract_price"]').closest('div').find('span');
	                var currencyField = form.find('input[name="contract_price"]');
	            }else if(moduleName == "Products" ){
	                var priceSymbol = form.find('input[name="purchase_cost"]').closest('div').find('span');
	                var currencyField = form.find('input[name="purchase_cost"]');
	            }else if(moduleName == "Services" ){
	                var priceSymbol = form.find('input[name="purchase_cost"]').closest('div').find('span');
	                var currencyField = form.find('input[name="purchase_cost"]');
	            }

	            var currency = form.find('select[name="currency"]');
            	...
            	/*2020/11/10 Pyae Phyo Mon Add currency in Quick Create Ver 1.0 Add End*/
...
//			});
	...
	openPopUp : function(e) {
		...
		app.event.one(Vtiger_Edit_Js.popupSelectionEvent,function(e,data) {
			...
			/*2020/09/22 Thet Phyo Wai Add Field Ticket Ver 1.0 Start*/
			//UPDATE STARt
			var parentModule = jQuery('#popupPageContainer').find('#parentModule').val();
			var sourceModule = jQuery('#popupPageContainer').find('#module').val();

			if(parentModule == 'HelpDesk' || parentModule == "ContactProduct" || parentModule == "Invoice"){
			  if(sourceModule=='Contacts'){
			  	if(parentModule == 'HelpDesk'){
			  		var accountElem = thisInstance.getParentElement(jQuery('input[name="parent_id"]'));
			  	}else if(parentModule == "ContactProduct"){
			  		var accountElem = thisInstance.getParentElement(jQuery('input[name="organization_name_display"]'));
			  	}else if(parentModule == "Invoice"){
			  		var accountElem = thisInstance.getParentElement(jQuery('input[name="account_id"]'));
			  	}
				var account = value.account_info;
				if(account != ""){
					var data = {
							'name' : account.accountname,
							'id' : account.accountid
						}
					thisInstance.setReferenceFieldValue(accountElem, data);
				}
				if(parentModule == 'HelpDesk'){
					registerClearReferenceTargetSelection(thisInstance.getParentElement(jQuery('input[name="product_id"]')));
			  		registerClearReferenceTargetSelection(thisInstance.getParentElement(jQuery('input[name="machine_serial_no"]')));
			 	}
			  }else if(parentModule == 'HelpDesk' && sourceModule=='Products'){
			  	registerClearReferenceTargetSelection(thisInstance.getParentElement(jQuery('input[name="machine_serial_no"]')));
			  }else if(parentModule == 'HelpDesk' && sourceModule=='Accounts'){
			  	registerClearReferenceTargetSelection(thisInstance.getParentElement(jQuery('input[name="product_id"]')));
			  	registerClearReferenceTargetSelection(thisInstance.getParentElement(jQuery('input[name="machine_serial_no"]')));
			  	registerClearReferenceTargetSelection(thisInstance.getParentElement(jQuery('input[name="contact_id"]')));
			  }


			function registerClearReferenceTargetSelection(e) {
				var parentTdElement = e.closest('td');
				var element = parentTdElement.find('.clearReferenceSelection');
				if(parentTdElement.length == 0){
					parentTdElement = element.closest('.fieldValue');
				}
				var inputElement = parentTdElement.find('.inputElement');
				...
				parentTdElement.find('.referencefield-wrapper').removeClass('selected');
				/*2020-12-07 Thet Phyo Wai Machine Serial No Update Readonly MSCRM Start*/
				/*inputElement.removeAttr("disabled").removeAttr('readonly');*/
...
//					
	...
	getPopUpParams : function(container) {
		...
		/*2020/09/08 Thet Phyo Wai Add Field Ticket Ver 1.0 Add Start*/
		//UPDATE START
		if(sourceModule == "HelpDesk" && (sourceField == 'contact_id' || sourceField == 'product_id' || sourceField == 'machine_serial_no') ){
			if(app.view() == 'Edit'){
				var form = this.getForm();
				var accountIdElement  = form.find('[name="parent_id"]');
				if(accountIdElement.length > 0 && accountIdElement.val().length > 0 && accountIdElement.val() != 0) {
                params['related_account_id'] = accountIdElement.val();
            	}
				var contactIdElement  = form.find('[name="contact_id"]');
				if(contactIdElement.length > 0 && contactIdElement.val().length > 0 && contactIdElement.val() != 0) {
                params['related_contact_id'] = contactIdElement.val();
            	}
            	var productIdElement  = form.find('[name="product_id"]');
            	if(productIdElement.length > 0 && productIdElement.val().length > 0 && productIdElement.val() != 0) {
                	params['related_product_id'] = productIdElement.val();
            	}
			}else if(app.view() == 'Detail'){
				var form = jQuery('#detailView');
				var accountIdElement  = form.find('[data-name="parent_id"]');
				if(accountIdElement.length > 0 && accountIdElement.val().length > 0 && accountIdElement.val() != 0) {
                params['related_account_id'] = accountIdElement.val();
            	}
				var parentIdElement  = form.find('[data-name="contact_id"]');
				if(parentIdElement.length > 0 && parentIdElement.data('value') != 0) {
                	params['related_contact_id'] = parentIdElement.data('value');
            	}
            	var productIdElement  = form.find('[data-name="product_id"]');
            	if(productIdElement.length > 0 && productIdElement.data('value') != 0) {
                	params['related_product_id'] = productIdElement.data('value');
            	}
			}
			if(sourceField == 'machine_serial_no'){
				params['related_parent_module'] = 'ContactProduct';
			}
    	}
    	//UPDATE END
