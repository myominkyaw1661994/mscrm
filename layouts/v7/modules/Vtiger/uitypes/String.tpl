...
{strip}
	...
	<input id="{$MODULE}_editView_fieldName_{$FIELD_NAME}" type="text" data-fieldname="{$FIELD_NAME}" data-fieldtype="string" class="inputElement {if $FIELD_MODEL->isNameField()}nameField{/if}" name="{$FIELD_NAME}" value="{$FIELD_MODEL->get('fieldvalue')}"
		...
		{/if}
		{*2020-09-30 Thet Phyo Wai Customize Invoice Module MSCRM Ver 1.0 Start*}
		{if $MODULE eq 'Invoice' and $FIELD_NAME eq 'ticket_number'}
		readonly="readonly"
		{/if}
		{*2020-09-30 Thet Phyo Wai Customize Invoice Module MSCRM Ver 1.0 END*}
		   />
{/strip}
