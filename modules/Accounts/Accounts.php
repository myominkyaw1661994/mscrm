...
class Accounts extends CRMEntity {
	...
	var $related_module_table_index = array(
		...
		// 2020/11/24 sulatt organization merge data lost problem MSCRM start
		'ContactProduct' => array('table_name' => 'vtiger_contactproduct', 'table_index' => 'contactproduct_id', 'rel_index' => 'organization_name'),
		// 2020/11/24 sulatt organization merge data lost problem MSCRM end
	);
	...
	*/
	/*2020-09-21 Thet Phyo Wai Account Number Field MSCRM Var 1.0 Start*/
	...
	{		
		...
		$relatedToFieldResult =$adb->query('SELECT account_no, account_number FROM vtiger_account WHERE accountid = '.$accountId);
		
		$fieldNo = $adb->query_result($relatedToFieldResult, 0, 'account_no');
		$fieldNumber = $adb->query_result($relatedToFieldResult, 0, 'account_number');
		
		$already_exit = false;
		// if($fieldNumber != ""){
		if($fieldNumber != "" && $_REQUEST['action'] != "ProcessDuplicates"){
			$Result = $adb->pquery("SELECT accountid FROM vtiger_account WHERE accountid !=? AND account_number =?", array($accountId,$fieldNumber));

			if($adb->getRowCount($Result) > 0) $already_exit = true;
		}


		if($fieldNumber == "" || $already_exit){
			$adb->pquery("UPDATE vtiger_account SET account_number = ? WHERE account_no = ?", array($fieldNo,$fieldNo));	
		}
			
	}
	/*2020-09-21 Thet Phyo Wai Account Number Field MSCRM Var 1.0 End*/
	...
	*/
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Contacts"=>"vtiger_contactdetails","Potentials"=>"vtiger_potential","Quotes"=>"vtiger_quotes",
					...
					"Assets"=>"vtiger_assets","Project"=>"vtiger_project", "Emails"=>"vtiger_seactivityrel","ContactProduct"=>"vtiger_contactproduct"); // 2020/11/24 sulatt organization merge data lost problem MSCRM
		...
		$tbl_field_arr = Array("vtiger_contactdetails"=>"contactid","vtiger_potential"=>"potentialid","vtiger_quotes"=>"quoteid",
					...
					"vtiger_assets"=>"assetsid","vtiger_project"=>"projectid","vtiger_seactivityrel"=>"activityid","vtiger_contactproduct"=>"contactproduct_id"); // 2020/11/24 sulatt organization merge data lost problem MSCRM
		...
		$entity_tbl_field_arr = Array("vtiger_contactdetails"=>"accountid","vtiger_potential"=>"related_to","vtiger_quotes"=>"accountid",
					...
					"vtiger_assets"=>"account","vtiger_project"=>"linktoaccountscontacts","vtiger_seactivityrel"=>"crmid","vtiger_contactproduct"=>"organization_name"); // 2020/11/24 sulatt organization merge data lost problem MSCRM
	...
	*/
	...
	function unlinkDependencies($module, $id) {
		...
		//Backup ContactProduct-Account Relation
		$con_q = 'SELECT contactproduct_id FROM vtiger_contactproduct WHERE organization_name = ?';
		$con_res = $this->db->pquery($con_q, array($id));
		if ($this->db->num_rows($con_res) > 0) {
			$con_ids_list = array();
			for($k=0;$k < $this->db->num_rows($con_res);$k++)
			...
			$params = array($id, RB_RECORD_UPDATED, 'vtiger_contactproduct', 'organization_name', 'contactproduct_id', implode(",", $con_ids_list));
			$this->db->pquery('INSERT INTO vtiger_relatedlists_rb VALUES(?,?,?,?,?,?)', $params);
		}
		//Deleting Contact-Account Relation.
		$con_q = 'UPDATE vtiger_contactproduct SET organization_name = 0 WHERE organization_name = ?';
		$this->db->pquery($con_q, array($id));
	...
	function save_related_module($module, $crmid, $with_module, $with_crmids, $otherParams = array()) {
		...
		foreach($with_crmids as $with_crmid) {
			...
			//2020-11-25 Pyae Phyo Mon update the crmentity is_update field update MSCRM ver1.0 Start
			elseif($with_module == 'Contacts') {				
				$adb->pquery("UPDATE vtiger_crmentity set is_update = 1 where crmid = ?", array($crmid));
			}	
			//2020-11-25 Pyae Phyo Mon update the crmentity is_update field update MSCRM ver1.0 End		
			else {
	...
	/*2020-12-30 Thet Phyo Wai Add Customer Product Link in Customer Portal MSCRM ADD Etart*/
	function get_contactproduct($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		global $log, $singlepane_view,$currentModule,$current_user;
		$log->debug("Entering get_contactproduct(".$id.") method ...");
		$this_module = $currentModule;
		...
		$query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name,
				vtiger_crmentity.*, vtiger_contactproduct.*, vtiger_potential.potentialname, vtiger_account.accountname
				FROM vtiger_contactproduct
				INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_contactproduct.contactproduct_id
				LEFT OUTER JOIN vtiger_account ON vtiger_account.accountid = vtiger_contactproduct.organization_name
				LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
                LEFT JOIN vtiger_contactproductcf ON vtiger_contactproductcf.contactproduct_id = vtiger_contactproduct.contactproduct_id
				LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid = vtiger_users.id
				WHERE vtiger_crmentity.deleted = 0 AND (vtiger_account.accountid = $id";

		if(!empty ($entityIds)){
			$query .= " OR vtiger_contactproduct.contact_id IN (".$entityIds."))";
		} else {
			$query .= ")";
		}
		...
		$log->debug("Exiting get_contactproduct method ...");
		return $return_value;
	}
	/*2020-12-30 Thet Phyo Wai Add Customer Product Link in Customer Portal MSCRM ADD End*/
}
