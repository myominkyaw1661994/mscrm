...
class CSCProducts_Save_Action extends Vtiger_Save_Action
{
	...
	function insertUnitInformation($cscproductsId, $product_units)
	{
		...
		$query = "INSERT INTO `vtiger_cscproductunitconversion`(`cscproductsid`, `usageunit`, `unitconversion`, `selling_price`, `currencyid`, `sequence`, `unit_qty`) VALUES (?,?,?,?,?,?,?)";
		foreach ($product_units as $key => $values) {
			$params = array($cscproductsId, $values['usageunit'], $values['unitconversion'], $values['selling_price'], $values['currencyid'], $values['sequence'], $values['qtyinstock']);
