...
class Activity extends CRMEntity {
	...
	function save_module($module)
	{
		...
		/*2020-11-01 Thet Phyo Wai Task Cannot Add From Contact MSCRM Ver 1.0 Start*/
		/*if(isset($this->column_fields['parent_id']) && $this->column_fields['parent_id'])*/
		if(isset($this->column_fields['parent_id']) && $this->column_fields['parent_id'] != '' && $this->column_fields['parent_id'] != 0)
		/*2020-11-01 Thet Phyo Wai Task Cannot Add From Contact MSCRM Ver 1.0 End*/
		{
			...
			$lead_value = Potentials_Module_Model::getPotentialsLead($this->column_fields['parent_id']);
			if($lead_value['is_potential']){

				$adb->pquery('UPDATE vtiger_activity SET lead_id = ? WHERE activityid = ?', array($lead_value['leadid'],$this->id));
			}
		...
		//Handling for invitees
			...
			/* 22-12-2020 Myo Min Kyaw Email Send When Event Create or Update MSCRM Ver 1.0 start*/
			$orgInviteUser = null;
			$oldUser = array();
			$newUser = array();
			$deleteUser = array();
			$invitees_array = explode(';',$selected_users_string);
			$result = $adb->pquery("SELECT inviteeid FROM `vtiger_invitees` WHERE activityid = ?", array($this->id));
			$numRows = $adb->num_rows($result);

			for($i = 0; $i < $numRows; $i++){
				$orgInviteUser[$i] = $adb->query_result($result, $i, "inviteeid");
			}
			
			for($i = 0; $i < count($invitees_array); $i++){
				if(in_array($invitees_array[$i], $orgInviteUser)){
					array_push($oldUser, $invitees_array[$i]);
				}else{
					array_push($newUser, $invitees_array[$i]);
				}
			}
			
			//30-12-2020 Myo Min Kyaw Event invite user disapper when edit from quick edit MSCRM Ver1.0 start
			if($_REQUEST[action] !== "SaveAjax" && $this->mode !== ""){
				for($i = 0; $i < count($orgInviteUser); $i++){
					if(!in_array($orgInviteUser[$i], $invitees_array)){
						array_push($deleteUser, $orgInviteUser[$i]);
					}
				}
			}

			if($_REQUEST[action] === "SaveAjax" && $this->mode === "edit"){
				$oldUser = $orgInviteUser;
			}
			//30-12-2020 Myo Min Kyaw Event invite user disapper when edit from quick edit MSCRM Ver1.0 end

			vglobal('ol', $oldUser);
			vglobal('ne', $newUser);
			vglobal('de', $deleteUser);
			/* 22-12-2020 Myo Min Kyaw Email Send when Event is create or update MSCRM Ver 1.0 end*/
