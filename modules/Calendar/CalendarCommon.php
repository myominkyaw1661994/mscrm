...
function getActivityDetails($description,$user_id,$from='',$recordModel=false) {
	...
	*/
	...
	/*22-12-2020 Myo Min Kyaw Email Send when event is creat or update MSCRM Ver1.0 end*/
	
	$oldUser = vglobal('oldUser');
	$newUser = vglobal('newUser');
	$deleteUser = vglobal('deleteUser');

	if($oldUser === "olduser" || $newUser === "newuser" || $deleteUser === "deleteuser"){
		if($oldUser === "olduser"){
				$query='SELECT body FROM vtiger_emailtemplates WHERE subject=? AND systemtemplate=?';
				$result = $db->pquery($query, array('Invitation2', '0'));
				$body=decode_html($db->query_result($result,0,'body'));
				$body=addAcceptEventLink($body,$user_id,$recordModel);
				$list = $body;
				$list = str_replace('$invitee_name$', $name, $list);
		...
		if($deleteUser === "deleteuser"){
			$query='SELECT body FROM vtiger_emailtemplates WHERE subject=? AND systemtemplate=?';
				$result = $db->pquery($query, array('Invitation3', '0'));
				$body=decode_html($db->query_result($result,0,'body'));
				// $body=addAcceptEventLink($body,$user_id,$recordModel);
				$list = $body;
				$list = str_replace('$invitee_name$', $name, $list);
...
/*22-12-2020 Myo Min Kyaw Email Send when event is creat or update MSCRM Ver1.0 start*/
function sendMailToUser($inviteeid, $recordModel, $desc, $subject, $attachment){
	if($inviteeid != '') {
...
function sendInvitation($inviteesid,$mode,$recordModel,$desc) {
	...
	$oldUser = vglobal('ol');
	$newUser = vglobal('ne');
	$deleteUser = vglobal('de');

	if(is_array($oldUser) || is_array($newUser) || is_array($deleteUser)){
		if($oldUser){
			vglobal("oldUser", "olduser");
			$subject = vtranslate("LBL_UPDATED_INVITATION", "Calendar").' : ';
			$subject .= $recordModel->get('subject');
			$attachment = generateIcsAttachment($desc);
			 foreach($oldUser as $inviteeid) {
				sendMailToUser($inviteeid, $recordModel, $desc, $subject, $attachment);
			 }
			 vglobal("oldUser", "");
			 unlink($attachment);
		 }
		 ...
		 if($newUser){
			 vglobal("newUser", "newuser");
			 $subject = vtranslate("LBL_INVITATION", "Calendar").' : ';
			 $subject .= $recordModel->get('subject');
			 $attachment = generateIcsAttachment($desc);
			 foreach($newUser as $inviteeid) {
				sendMailToUser($inviteeid, $recordModel, $desc, $subject, $attachment);
			 }
			 vglobal("newUser", "");
			 unlink($attachment);
		 ...
		 if($deleteUser){
			vglobal("deleteUser", "deleteuser");
			$subject = 'Invitation Cancel'.' : ';
			$subject .= $recordModel->get('subject');
			$attachment = generateIcsAttachment($desc);
			foreach($deleteUser as $inviteeid) {
			   sendMailToUser($inviteeid, $recordModel, $desc, $subject, $attachment);
			}
			vglobal("newUser", "");
			unlink($attachment);
