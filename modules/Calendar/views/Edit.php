...
Class Calendar_Edit_View extends Vtiger_Edit_View {
	...
	function Events($request, $moduleName) {
		...
		if($isRelationOperation) {
			...
			if($request->get('sourceModule') == 'Potentials'){
				$viewer->assign('HIDE_LEAD', true);
				$lead_value = Potentials_Module_Model::getPotentialsLead($request->get('sourceRecord'));
				$viewer->assign('LEAD_VALUE', $lead_value);
			}
			/*2020-10-14 Thet Phyo Wai Update Opportunity Activity MSCRM Ver 1.0 End*/
		}

		/*2020-10-14 Thet Phyo Wai Update Opportunity Activity MSCRM Ver 1.0 Start*/
		if(!empty($record) && !($recordModel->get('parent_id') =="" || $recordModel->get('parent_id') == 0 ) ){
			$lead_value = Potentials_Module_Model::getPotentialsLead($recordModel->get('parent_id'));
	...
	function Calendar($request, $moduleName) {
		...
		if(!empty($record)) {
			$recordModel = Vtiger_Record_Model::getInstanceById($record, $moduleName);
			if(!($recordModel->get('parent_id') =="" || $recordModel->get('parent_id') == 0 ) ){
				$lead_value = Potentials_Module_Model::getPotentialsLead($recordModel->get('parent_id'));
				if($lead_value['is_potential']){
					$viewer->assign('HIDE_LEAD', true);
					$viewer->assign('LEAD_VALUE', $lead_value);
