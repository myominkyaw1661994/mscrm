...
class Contacts extends CRMEntity {
	...
	var $related_module_table_index = array(
        ...
        // 2020/11/24 sulatt contact merge data lost problem MSCRM start
        'ContactProduct' => array('table_name' => 'vtiger_contactproduct', 'table_index' => 'contactproduct_id', 'rel_index' => 'organization_name'),
		// 2020/11/24 sulatt contact merge data lost problem MSCRM end
	);
  ...
  $log->debug("Exiting get_contactsforol method ...");
	...
	*/
	function save_module($module)
	{
		...
		/*2020-10-27 Thet Phyo Wai Update Contact For First Name and Contact Number MSCRM Ver 1.0 Start*/
		global $adb;
		$contactid = $this->id;

		// 2020-12-17 Pyae Phyo Mon When Contact Merge, Organization Update Ver 1.0 add Start
		if($_REQUEST['action'] == "ProcessDuplicates")
		{
			$OrgID1 = $_REQUEST['Merge_OrgID_1'];
			$OrgID2 = $_REQUEST['Merge_OrgID_2'];

			$mergecount = $_REQUEST['MergeCount'];
			if($mergecount == "3")
			{
				$OrgID3 = $_REQUEST['Merge_OrgID_3'];
				$adb->pquery("UPDATE vtiger_crmentity SET is_update = 1 where crmid in(?,?,?)", array($OrgID1,$OrgID2,$OrgID3));
			}
			else
			{
				$adb->pquery("UPDATE vtiger_crmentity SET is_update = 1 where crmid in(?,?)", array($OrgID1,$OrgID2));
			}
		}
		...
		$prevAccountID = $_REQUEST['old_account_id'];
		$currentAccountID = $_REQUEST['account_id'];
		if($prevAccountID != "" && $prevAccountID!=$currentAccountID){
			$adb->pquery("UPDATE vtiger_crmentity SET is_update = 1 where crmid = ?", array($prevAccountID));
		}
		/*2020-10-01 Thet Phyo Wai Update Contact Save MSCRM Ver 1.0 End*/

		$relatedToFieldResult = $adb->pquery("SELECT contact_no,contact_number,firstname from vtiger_contactdetails WHERE contactid=?", array($contactid));

		$fieldNo = $adb->query_result($relatedToFieldResult, 0, 'contact_no');
		$fieldNoOne = $adb->query_result($relatedToFieldResult, 0, 'contact_number');
		$firstname = $adb->query_result($relatedToFieldResult, 0, 'firstname');
		
		$already_exit = false;
		if($fieldNoOne != ""){
			$Result = $adb->pquery("SELECT contactid FROM vtiger_contactdetails WHERE contactid !=? AND contact_number =?", array($contactid,$fieldNoOne));
		...
		if($fieldNoOne == "" || $already_exit){
			$adb->pquery("update vtiger_contactdetails set contact_number=? where contact_no=?", array($fieldNo,$fieldNo));
		}

		if($firstname == NULL){
			$adb->pquery("update vtiger_contactdetails set firstname='' where contact_no=?", array($fieldNo));
		}
		/*2020-10-27 Thet Phyo Wai Update Contact For First Name and Contact Number MSCRM Ver 1.0 End*/
	}
	...
	*/
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("Potentials"=>"vtiger_contpotentialrel","Potentials"=>"vtiger_potential","Activities"=>"vtiger_cntactivityrel",
				...
				'Vendors'=>'vtiger_vendorcontactrel',"ContactProduct"=>"vtiger_contactproduct"); // 2020/11/24 sulatt contact merge data lost problem MSCRM
		...
		$tbl_field_arr = Array("vtiger_contpotentialrel"=>"potentialid","vtiger_potential"=>"potentialid","vtiger_cntactivityrel"=>"activityid",
				...
				'vtiger_vendorcontactrel'=>'vendorid',"vtiger_contactproduct"=>"contactproduct_id"); // 2020/11/24 sulatt contact merge data lost problem MSCRM start
		...
		$entity_tbl_field_arr = Array("vtiger_contpotentialrel"=>"contactid","vtiger_potential"=>"contact_id","vtiger_cntactivityrel"=>"contactid",
				...
				'vtiger_vendorcontactrel'=>'contactid',"vtiger_contactproduct"=>"contact_id"); // 2020/11/24 sulatt contact merge data lost problem MSCRM start
	...
	function unlinkDependencies($module, $id) {
		...
		$con_q = 'SELECT contactproduct_id FROM vtiger_contactproduct WHERE contact_id = ?';
		...
		if ($this->db->num_rows($con_res) > 0) {
			...
			$params = array($id, RB_RECORD_UPDATED, 'vtiger_contactproduct', 'contact_id', 'contactproduct_id', implode(",", $con_ids_list));
			$this->db->pquery('INSERT INTO vtiger_relatedlists_rb VALUES(?,?,?,?,?,?)', $params);
		}
		//Deleting ContactProduct-Contact Relation.
		$con_q = 'UPDATE vtiger_contactproduct SET contact_id = 0 WHERE contact_id = ?';
