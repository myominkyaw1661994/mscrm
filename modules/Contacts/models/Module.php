...
class Contacts_Module_Model extends Vtiger_Module_Model {
	...
	public function getCalendarActivities($mode, $pagingModel, $user, $recordId = false) {
		...
		/*2020-10-13 Thet Phyo Wai Complete Activity MSCRM Ver 1.0 Start*/
		...
		/*2020-10-13 Thet Phyo Wai Complete Activity MSCRM Ver 1.0 End*/
...
/*2020/09/08 Thet Phyo Wai Add Field Ticket Ver 1.0 Add Start*/
	function getAccountName($contactid) {
        $db = PearDatabase::getInstance();
        $query = "SELECT vtiger_account.accountid,vtiger_account.accountname FROM vtiger_account
        ...
        $newRow = '';
        if($numOfRows > 0){
        	$newRow = $db->query_result_rowdata($result, 0);
        	$newRow["module"] = "Accounts";
        }
        return $newRow;
	}
/*2020/09/08 Thet Phyo Wai Add Field Ticket Ver 1.0 Add End*/
