		...
		if ($current_user) {
			...
			if ($module == 'Faq') {
				...
				if ($mode == 'mine') {
					...
					/*2021-01-08 Thet Phyo Wai Add Customer Product Link Module MSCRM Start*/
					/*$limitClause = sprintf('ORDER BY %s %s LIMIT %s,%s', $orderBy, $order, ($page * $pageLimit), $pageLimit);*/
					if($module == "ContactProduct"){
						$limitClause = sprintf('ORDER BY id DESC LIMIT %s,%s', ($page * $pageLimit), $pageLimit);
					...
					/*2021-01-08 Thet Phyo Wai Add Customer Product Link Module MSCRM End*/
