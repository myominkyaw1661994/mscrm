	...
	function process(CustomerPortal_API_Request $request) {
		...
		if ($current_user) {
			...
			if (!empty($searchKey)) {
				...
				/*2021-01-05 Thet Phyo Wai Add Customer Product Link in Customer Portal MSCRM Start*/
				if($module == 'Products'){
					$contactproduct_sql = sprintf("SELECT product_name FROM ContactProduct WHERE customer_active ='Yes' AND contact_id = '%s';", $contactWebserviceId);
					$result = vtws_query($contactproduct_sql, $this->getActiveUser());
					$result_string = "";
					foreach ($result as $value) {
						if(empty($result_string)){
							$result_string = "'".$value['product_name']."'";
						}else{
							$result_string .= ",'".$value['product_name']."'";
						}
					}
					$sql .= sprintf(" AND id IN (%s) ", $result_string);
				}
				/*2021-01-05 Thet Phyo Wai Add Customer Product Link in Customer Portal MSCRM Start*/
			}
