			...
			if (in_array($module, array('HelpDesk', 'Documents', 'Assets', 'Quotes', 'Contacts', 'Accounts'))) {
				...
				if ($module !== 'Contacts' && $module !== 'Accounts') {
					...
					if ($module == 'HelpDesk') {
						...
						if(empty($recordId)){
							$groupId = CustomerPortal_Utils::getEngineeringGroupId();
							if($groupId){
								$editableFields[] = 'assigned_user';
								$productId = explode("x", $values['product_id']);
								$values['assigned_user'] = $productId[0].'x'.$groupId;
							}
						}
