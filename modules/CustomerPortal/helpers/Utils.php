...
class CustomerPortal_Utils {
	...
	static function getImageDetails($recordId, $module) {
		...
		/*2021-01-07 Thet Phyo Wai Customer Portal Cannot Show Contact Picture MSCRM Start*/
		/*$imageName = $adb->query_result($result, 0, 'name');*/
		$imageName = $adb->query_result($result, 0, 'storedname');
		/*2021-01-07 Thet Phyo Wai Customer Portal Cannot Show Contact Picture MSCRM End*/
	...
	public static function isModuleActive($module) {
		...
		$defaultAllowedModules = array("ModComments", "History", "Contacts", "Accounts","Export");
	...
	/*2021-01-19 Thet Phyo Wai Add Default Assign User to Engineering Group MSCRM Start*/
	static function getEngineeringGroupId() {
		global $adb;
		$groupIdResult = $adb->pquery("SELECT default_assignee FROM vtiger_customerportal_settings", array());
		if ($adb->num_rows($groupIdResult)) {
			$groupId = $adb->query_result($groupIdResult, 0, 'default_assignee');
			return $groupId;
		}else{
			return false;
		}
	}
	/*2021-01-19 Thet Phyo Wai Add Default Assign User to Engineering Group MSCRM End*/
