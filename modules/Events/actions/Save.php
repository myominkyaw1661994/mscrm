...
class Events_Save_Action extends Calendar_Save_Action {
	...
	public function saveRecord($request) {
		...
		$rr = $request->get('sourceModule');
		$ss = $request->get('sourceRecord');
		if($request->get('relationOperation')) {
			...
			if($parentModuleName == "HelpDesk"){
				...
				addLastActiveFieldInTicket($pId);
			}
		...
		//2021-07-05 Myo Min Kyaw update the last active fieldd in ticket activity create start
		if($request->get('returnmodule') == "HelpDesk" && $request->get('module') == "Events" && $request->get('returnrelatedModule') == "Calendar"){
			$pid = $request->get('returnrecord');
			addLastActiveFieldInTicket($pid);		
		}
		//2021-07-05 Myo Min Kyaw update the last active fieldd in ticket activity create end
