...
class HelpDesk_Module_Model extends Vtiger_Module_Model {
	...
	function get_ticket_products($recordModel, $relatedModuleModel) {
		$query = 'SELECT vtiger_products.productid, vtiger_products.productname, vtiger_products.productcode, vtiger_products.commissionrate,
						...
						vtiger_ticketsproductrel.qty,
						vtiger_ticketsproductrel.listprice,
						vtiger_ticketsproductrel.usedcurrency
				FROM vtiger_products
				INNER JOIN vtiger_ticketsproductrel ON vtiger_products.productid = vtiger_ticketsproductrel.productid
				INNER JOIN vtiger_crmentity on vtiger_crmentity.crmid = vtiger_products.productid
				INNER JOIN vtiger_troubletickets on vtiger_troubletickets.ticketid = vtiger_ticketsproductrel.ticketid
				...
				WHERE vtiger_troubletickets.ticketid = '.$recordModel->getId().' and vtiger_crmentity.deleted = 0';
		return $query;
	}

	function get_ticket_services($recordModel, $relatedModuleModel) {
		$query = 'SELECT vtiger_service.serviceid, vtiger_service.servicename, vtiger_service.service_no, vtiger_service.commissionrate,
					...
					vtiger_ticketsproductrel.usedcurrency
			FROM vtiger_service
			INNER JOIN vtiger_ticketsproductrel on vtiger_service.serviceid = vtiger_ticketsproductrel.productid
	...
	/*2020-10-03 Thet Phyo Wai Ticket Generate Invoice MSCRM Ver 1.0 Add Start*/
	function getGenerateInvoice($ticket_no) {
        global $adb;
        if($ticket_no != ""){
	        $query = "SELECT * FROM vtiger_invoice
	                    INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_invoice.invoiceid
	                    WHERE vtiger_crmentity.deleted = 0 AND vtiger_invoice.ticket_number = ?";

	        $result = $adb->pquery($query, array($ticket_no));
