...
class ITS4YouReports_IndexAjax_Action extends Vtiger_Action_Controller {
    ...
    function __construct() {
        ...
        $this->exposeMethod('installDownload');
        $this->exposeMethod('checkCsvPath');
    }
...
//echo "<pre>";print_r($return_array);echo "</pre>";
        ...
        if (!empty($picklistValues)) {
            foreach ($picklistValues as $order => $pickListValue) {
                ...
                if ($uitype_row['uitype'] == '56') {
                    $sel_fields[$ni] = [vtranslate($pickListValue, $s_module_field_arr[0]), $order, $chk_val];
                } else {
                    $sel_fields[$ni] = [vtranslate($pickListValue, $s_module_field_arr[0]), $pickListValue, $chk_val];
                }
                $ni++;
            }
            if ($pickcount == 0 && !empty($value)) {
                $sel_fields[$ni] = [vtranslate('LBL_NOT_ACCESSIBLE'), $value, 'selected'];
            }
        }
...
//error_reporting(63);ini_set("display_errors",1);
    ...
    private static function checkWritable()
    {
        $returnNotWritable = [];
        $wArray = [
            'test/',
            'test/ITS4YouReports/',
            'modules/ITS4YouReports/highcharts/',
        ];

        foreach ($wArray as $wPath) {
            if (!is_writable($wPath)) {
                $returnNotWritable[] = $wPath;
            }
        }

        return $returnNotWritable;
    }
    // ITS4YOU-END
    function installDownload(Vtiger_Request $request)
    {
        $done = false;

        GenerateObj::createTempFolderIfNotExists();

        $notWritable = self::checkWritable();
        ...
        if (empty($notWritable)) {
            ...
            if ('v7' === $layout) {
                $url = 'https://www.its4you.sk/en/images/extensions/Reports4You/src/highcharts_7x.zip';
            } else {
                $url = 'https://www.its4you.sk/en/images/extensions/Reports4You/src/highcharts.zip';
            }

            $zipFile = 'test/ITS4YouReports/highcharts.zip'; // Rename .zip file
            $extractDir = 'modules/ITS4YouReports/highcharts/'; // Name of the directory where files are extracted
            ...
            if ($done) {
                $result = ['success' => true, 'message' => vtranslate('LBL_DONE', $request->getModule())];
            } else {
                $result = ['success' => false, 'message' => vtranslate('LBL_COPPY_FAILED', $request->getModule())];
            }
        } else {
            $result = [
                'success' => false,
                'message' => vtranslate('LBL_IMPORT_DIRECTORY_NOT_WRITABLE', 'Import') .
                    PHP_EOL .
                    implode(PHP_EOL, $notWritable)
            ];
        }
     ...
     */
    function checkCsvPath(Vtiger_Request $request)
    {
        $csvPath = ITS4YouScheduledReport::prepareCsvPath($request->get('csv_path'));

        if (!is_writable($csvPath)) {
            $result = [
                'success' => false,
                'message' => vtranslate('LBL_CSV_DIRECTORY_NOT_WRITABLE', $request->getModule())
            ];
        } else {
            $result = ['success' => true];
        }
