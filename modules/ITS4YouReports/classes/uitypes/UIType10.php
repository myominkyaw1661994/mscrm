...
// show("oldo ui 10 joining"," $uirel_table_name AS $uirel_table_name".$fieldid_alias." ",$join_array,"oldo ui 10 joining post");
    ...
    public function getJoinSQL(&$join_array, &$columns_array) {
        ...
        $fieldid_alias = '';
        $fieldidAliasRes = $adb->pquery(
            'SELECT fieldid FROM vtiger_field WHERE tablename=? AND fieldname=?',
            [vtlib_purify($this->params['tablename']), vtlib_purify($this->params['columnname'])]
        );
        $fieldid_alias_row = $adb->fetchByAssoc($fieldidAliasRes, 0);
        if (!empty($fieldid_alias_row) && $fieldid_alias_row["fieldid"] != "") {
            $fieldid_alias = "_" . $fieldid_alias_row["fieldid"];
        ...
        if('vtiger_modcomments' === $this->params['tablename'] && in_array($this->params['columnname'], ['userid', 'customer',])){
            $fieldid_alias = $old_oth_fieldid = $mif_as = "";
            if ($this->params["fieldid"] != "") {
                $fieldid_alias = "_" . $this->params["fieldid"];
            }
        ...
        } elseif('vtiger_modcomments' === $this->params['tablename']) {
            if ('parent_comments' === $this->params['fieldname']) {
                $ui10_res = $adb->pquery('
                    SELECT relmodule 
                ...
                $ui10_res = $adb->pquery('
                    SELECT relmodule 
                        ...
                        WHERE fieldid = ? 
                        AND isentitytype = 1
                        AND relmodule IN (SELECT vtiger_crmentity.setype 
                        FROM `vtiger_modcomments` 
                        INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_modcomments.related_to 
                        GROUP BY setype)', [
                    $fieldid
                ]);
            }
    ...
    public function getJoinSQLbyFieldRelation(&$join_array, &$columns_array) {
        ...
        if(!in_array($this->params["columnname"], ITS4YouReports::$intentory_fields)){
            $adb = PEARDatabase::getInstance();
            $uirelRes = $adb->pquery('SELECT *  FROM vtiger_field WHERE tabid = ? AND fieldname = ?', [$r_tabid, $params_fieldname]);
            $uirel_row = $adb->fetchByAssoc($uirelRes, 0);
        }
...
//        }
        ...
        if (isset($this->params["fieldid"]) && $this->params["fieldid"] != "") {
            ...
            $fieldid = $this->params["fieldid"];
            $adb = PEARDatabase::getInstance();
            $stjoinRes = $adb->pquery('SELECT tablename, columnname FROM vtiger_field WHERE fieldid = ?', [$fieldid]);
            $stjoin_row = $adb->fetchByAssoc($stjoinRes, 0);
            $tablename = $stjoin_row["tablename"];
            $columnname = $stjoin_row["columnname"];
...
// show("oldo ui 10 intentory join000",$join_array,"oldo ui 10 intentory join000 POST");
        ...
        if (!empty($this->params["columnname"])) {
            ...
            if ('vtiger_crmentity' !== $column_tablename_alias) {
    ...
    public function getSelectedFieldCol($selectedfields) {
        ...
        if (isset($this->columns_array[$this->params['columnname']]['fld_cond']) && !empty($this->columns_array[$this->params['columnname']]['fld_cond'])) {
            $return = $this->columns_array[$this->params['columnname']]['fld_cond'];
        } else {
