...
class UITypeDefault extends UITypes {
    ...
    public function getJoinSQL(&$join_array, &$columns_array) {
        $old_oth_as = $old_oth_fieldid = $mif_as = $fld_hrefid = $rel_tabid = $access_count_sql = "";
...
//ITS4YouReports::sshow($params);
        ...
        if($this->params["tablename"]=="vtiger_modtracker_detail" || $this->params["tablename"]=="vtiger_modtracker_basic"){
            ...
            switch ($this->params["columnname"]) {
                ...
                case 'prevalue':
                case 'postvalue':
                    if (!in_array('vtiger_modtracker_detail.module', $columns_array)) {
                        $columns_array[] = 'vtiger_modtracker_basic.module';
                    }
                    if (!in_array('vtiger_modtracker_detail.fieldname AS fieldname', $columns_array)) {
                        $columns_array[] = 'vtiger_modtracker_detail.fieldname AS fieldname';
                    }
            ...
            if (!array_key_exists($join_tablename_alias, $join_array)) {
                ...
                if (!empty($this->params['using_array']['using'])) {
                    $using_tablename = $this->params['using_array']['using']['tablename'];
                    $utbl = explode($old_oth_as . $old_oth_fieldid . $mif_as, $using_tablename);
                    $utblName = $utbl[0];
                    $using_columnname = $foc->tab_name_index[$utblName];
                } else {
                    $using_tablename = $this->params['primary_table_name'];
                    $using_columnname = $this->params['primary_table_index'];
                }
...
//        }
        ...
        $join_array[" vtiger_servicecf AS vtiger_servicecf_inv" . $fieldid_alias . " "]["joincol"] = "vtiger_servicecf_inv" . $fieldid_alias . ".serviceid";
        $join_array[" vtiger_servicecf AS vtiger_servicecf_inv" . $fieldid_alias . " "]["using"] = "vtiger_service_inv" . $fieldid_alias . ".serviceid";
