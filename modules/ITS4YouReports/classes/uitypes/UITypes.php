...
class UITypes {
    ...
    public function getJoinSQLbyFieldRelation(&$join_array, &$columns_array) {
        ...
        if (empty($this->params['columnname']) && !empty($this->params['fieldid'])) {
            $fieldRes = $adb->pquery('SELECT fieldname, columnname, tablename  FROM vtiger_field WHERE fieldid = ?', array($this->params['fieldid']));
            $fieldRow = $adb->fetchByAssoc($fieldRes, 0);
        ...
        //$adb->setDebug(1);
        $uirel_res = $adb->pquery('SELECT *  FROM vtiger_field WHERE tabid = ? AND fieldname = ?', array($r_tabid, $params_fieldname));
        $uirel_row = $adb->fetchByAssoc($uirel_res, 0);
        //$adb->setDebug(0);
    ...
    protected function getInventoryColumnFldCond($columnName,$column_tablename_alias,$fieldid_alias="",$primary_table_name=""){
        switch ($columnName) {
            ...
            // ITS4YOU-END 
            case 'purchase_cost':
                $fld_cond = strtolower($this->params['tablename'] . $fieldid_alias . '.' . $this->params['columnname']);
                break;
