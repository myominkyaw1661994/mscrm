...
function getSecondaryColumns_GroupingHTML($moduleid, $selected = "", $ogReport = "") {
    ...
    if ($module_id != "") {
        ...
        if ($field_id != "" && !in_array($field_id, ITS4YouReports::$customRelationTypes)) {
            $fieldname_result = $adb->pquery("SELECT fieldlabel,uitype FROM vtiger_field WHERE fieldid=?", array($field_id));
            $fieldname_row = $adb->fetchByAssoc($fieldname_result, 0);
...
function sgetNewColumnstoTotalHTMLScript($Objects) {
    ...
    foreach ($Objects as $key => $ObjectRow) {
        ...
        if ($clear_tablename != "") {
            ...
            $sc_field_result = $adb->pquery("SELECT uitype FROM vtiger_field WHERE tablename = ? and columnname = ? and tabid=?", array($clear_tablename, $sc_columnname, $sc_module_id));
            $sc_field_row = $adb->fetchByAssoc($sc_field_result, 0);
        ...
        if (isset($result)) {
            foreach ($result as $key => $value) {
                ...
                if (isset($mod_strings[$value])) {
                    $Options[] = array("value" => $key . "$fieldid", 'text' => vtranslate($module, $module) . " - " . vtranslate($value, $value));
                } else {
...
function getPrimaryTLStdFilter($module, $ogReport = "") {
    ...
    if (vtlib_isModuleActive($module)) {
        ...
        if (isset($result)) {
            foreach ($result as $key => $value) {
                $fieldid = "";
                $Options[vtranslate($module, $module)][] = array("value" => $key . "$fieldid", 'text' => $value . " - " . vtranslate($value, $value));
            }
        }
    }
    ...
    if (ITS4YouReports::isInventoryModule($module)) {
        ...
        if ('v7' === $layout) {
            $fields = array_merge(
                $fields,
                [
                    'purchase_cost' => vtranslate('Purchase Cost', $module),
                    'margin'        => vtranslate('Margin', $module),
                ]
            );
        }
        $fields = array_merge(
            $fields,
            [
                'ps_producttotal'               => vtranslate('LBL_PRODUCT_TOTAL', $currentModule),
                'discount'                      => vtranslate('Discount', $module),
                'ps_productstotalafterdiscount' => vtranslate('LBL_PRODUCTTOTALAFTERDISCOUNT', $currentModule),
                'ps_productvatsum'              => vtranslate('LBL_PRODUCT_VAT_SUM', $currentModule),
                'ps_producttotalsum'            => vtranslate('LBL_PRODUCT_TOTAL_VAT', $currentModule),
            ]
        );
        global $site_URL;
        if (false !== strpos($site_URL, 'z-company')) {
            $fields['ps_profit'] = vtranslate('Profit', $currentModule);
        }
        $fields_datatype = [
            'listprice' => 'I',
            'quantity'  => 'I',
...
//$adb->setDebug(false);
    ...
    if (ITS4YouReports::isInventoryModule($module)) {
        ...
        $fields = array_merge(
            $fields,
            [
                'ps_producttotal'               => vtranslate('LBL_PRODUCT_TOTAL', $currentModuleName),
                'discount'                      => vtranslate('Discount', $module),
                'ps_productstotalafterdiscount' => vtranslate('LBL_PRODUCTTOTALAFTERDISCOUNT', $currentModuleName),
                'ps_productvatsum'              => vtranslate('LBL_PRODUCT_VAT_SUM', $currentModuleName),
                'ps_producttotalsum'            => vtranslate('LBL_PRODUCT_TOTAL_VAT', $currentModuleName),
            ]
        );
        global $site_URL;
        if (false !== strpos($site_URL, 'z-company')) {
            $fields['ps_profit'] = vtranslate('Profit', $currentModuleName);
        }
        $fields_datatype = [
            'listprice' => 'I',
            'quantity'  => 'I',
