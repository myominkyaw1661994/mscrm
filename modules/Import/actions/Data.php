...
class Import_Data_Action extends Vtiger_Action_Controller {
	...
	public function transformForImport($fieldData, $moduleMeta, $fillDefault = true, $checkMandatoryFieldValues = true) {
		...
		/*2020-06-25 Thet Phyo Wai Import failed error at Sales Order start*/
		$is_saleorder = false;
		if( $this->module == 'SalesOrder'){
			$is_saleorder = true;
			$enable_data = $fieldData['enable_recurring'];
		}
		/*2020-06-25 Thet Phyo Wai Import failed error at Sales Order end*/
		...
		if ($fieldData != null && $checkMandatoryFieldValues) {
			foreach ($moduleFields as $fieldName => $fieldInstance) {
				if ((($fieldData[$fieldName] == '') || ($fieldData[$fieldName] == null)) && $fieldInstance->isMandatory()) {
					...
					/*2020-06-25 Thet Phyo Wai Import failed error at Sales Order start*/
					/*return null;*/
					if($is_saleorder && ($fieldName == 'recurring_frequency' || $fieldName == 'start_period' || $fieldName == 'end_period' || $fieldName == 'payment_duration' || $fieldName == 'invoicestatus')){
						if( !($enable_data == 0 || $enable_data == '' || $enable_data == null) ){
							return null;
						}
					}else{
						return null;
