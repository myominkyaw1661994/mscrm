...
class Inventory_Record_Model extends Vtiger_Record_Model {
	...
	public function getParentRecordRelatedLineItems($parentRecordModel) {
		...
		foreach ($productDetails as $key => $lineItemDetail) {
			...
			if($parentRecordModel->getModuleName() === 'HelpDesk'){
				$CurrencySymbol = getCurrencySymbolandCRate($currencyId);

				$usedcurrency = $productDetails[$key]['usedcurrency'.$key];
				$usedCurrencySymbol = getCurrencySymbolandCRate($usedcurrency);
				$listprice = (float)$productDetails[$key]['listPrice'.$key] * ($CurrencySymbol['rate']/$usedCurrencySymbol['rate']);
				$productDetails[$key]['listPrice'.$key] = number_format($listprice, $numOfCurrencyDecimals, '.', '');
			}else{
				$productDetails[$key]['listPrice'.$key] = number_format((float)$convertedPriceDetails[$entityType][$productId], $numOfCurrencyDecimals, '.', '');
			}
			...
			$purchaseCurrencyId = $lineItemDetail['currency'.$key];
			$purchaseCurrency = getCurrencySymbolandCRate($purchaseCurrencyId);
			$purchaseCost = (float)$lineItemDetail['purchaseCost'.$key] * ((float)$userCurrencyInfo['conversion_rate'] / (float)$purchaseCurrency['rate']);
			/*2020-11-09 Thet Phyo Wai Purchase Cost Currency Change Start*/
	...
	public function getProductsForPurchaseOrder() {
		...
		for ($i = 1; $i <= $productsCount; $i++) {
			...
			$purchaseCurrencyId = $relatedProducts[$i]['currency'.$i];
			$purchaseCurrency = getCurrencySymbolandCRate($purchaseCurrencyId);
			$relatedProducts[$i]['listPrice'.$i] = ($relatedProducts[$i]['purchaseCost'.$i] / (float)$purchaseCurrency['rate']) / $relatedProducts[$i]['qty'.$i];
			$relatedProducts[$i]['purchaseCost'.$i] = $relatedProducts[$i]['listPrice'.$i];
