...
class PDFMaker_License_Action extends Vtiger_Action_Controller
{
    ...
    public function checkLicense($c = false)
    {
        ...
        if ($db_lic != "P" . md5($version_type . "/" . $salt . "\vt7")) {
            $mismatch += 1;
        }
        if ($DB_Licinfo[0] != $this->getCompanyInfo($version_type, $key)) {
            $mismatch += 1;
        }
        if ($DB_Licinfo[1] != $this->getUserKey($version_type, $key)) {
            $mismatch += 1;
        }
    ...
    public function editLicense(Vtiger_Request $request)
    {
        ...
        global $$sl;
        if (!$$sl) {
            $$sl = LoggerManager::getLogger('SOAP');
        }
        ...
        if (class_exists("Vtiger_Net_Client")) {
            ...
            if ($key != "") {
                ...
                if (isset($Validate["error"]) && !empty($Validate["error"])) {
                ...
                } elseif ($Validate["validate"] == "validated") {
                    ...
                    $adb->pquery("INSERT INTO vtiger_pdfmaker_version (version,license) VALUES (?,?)", array($vcv, "P" . md5($type . "/" . $salt . "\vt7")));
