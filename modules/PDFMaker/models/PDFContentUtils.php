...
class PDFMaker_PDFContentUtils_Model extends Vtiger_Base_Model
{
    ...
    public function getUserImage($id)
    {
        if (isset($id) and $id != "") {
            $db = PearDatabase::getInstance();
            $result = $db->pquery("select vtiger_attachments.* from vtiger_attachments left join vtiger_salesmanattachmentsrel on vtiger_salesmanattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid where vtiger_salesmanattachmentsrel.smid=?", array($id));
    ...
    public function getProductImage($id, $site_url, $productId = false, $sequence='1')
    {
        ...
        list($images, $bacImages) = $this->getInventoryImages($id, $isProductModule);
