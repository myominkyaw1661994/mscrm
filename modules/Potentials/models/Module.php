...
class Potentials_Module_Model extends Vtiger_Module_Model {
    ...
    function getPotentialsLead($potentialid) {
		global $adb;
		if($potentialid != "" ||$potentialid != "0"){ 
			$sql = "SELECT * FROM vtiger_potential WHERE potentialid = ?";
			$result = $adb->pquery($sql, array($potentialid));
			$num_rows = $adb->num_rows($result);
			$array['is_potential'] = ($num_rows>0)? true:false;

			$sql = "SELECT ld.leadid,ld.firstname,ld.lastname FROM vtiger_leaddetails AS ld 
			INNER JOIN vtiger_potential AS pt ON pt.lead_id = ld.leadid WHERE pt.potentialid = ?";
			$result = $adb->pquery($sql, array($potentialid));
			$num_rows = $adb->num_rows($result);
			if($num_rows > 0){
				$array['leadid'] = $adb->query_result($result, 0, 'leadid');
				$firstname = $adb->query_result($result, 0, 'firstname');
				$lastname = $adb->query_result($result, 0, 'lastname');
				$array['leadname'] = ($firstname) ? $firstname.' '.$lastname : $lastname;
			}
			return $array;
		}
	}
