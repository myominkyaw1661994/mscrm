...
class Products extends CRMEntity {
	...
	function save_module($module)
	{
		...
		// 2020-09-30 Myo Min Kyaw  UPdate the product no CRM Ver 1.0 add start
		global $adb, $current_user;
		$productid = $this->id;
		
		/*2020-11-05 Thet Phyo Wai Purchase Cost Currency Change Start*/
		if(!($_REQUEST['purchase_cost'] == 0 || $_REQUEST['purchase_cost'] == '' || $_REQUEST['purchase_cost'] == null)){
			$seperator = $current_user->currency_grouping_separator;
			$purchaseCostArr = explode($seperator,$_REQUEST['purchase_cost']);
			$purchaseCost = "";
			foreach($purchaseCostArr as $value){
				$purchaseCost .= $value;
			}
			$adb->pquery("UPDATE vtiger_products SET purchase_cost=? WHERE productid=?", array($purchaseCost,$productid) );
		}
		/*2020-11-05 Thet Phyo Wai Purchase Cost Currency Change Start*/

		//*2020-11-24 Pyae Phyo Mon Update is_create=true after transfer, change transfer to WD field value Save MSCRM Ver 1.0 Start*/
		if($_REQUEST['action'] == "Save")
		{
			$prevtransfertowd = $_REQUEST['old_transfer_to_wd'];
			$productResult = $adb->pquery("SELECT transfer_to_wd FROM vtiger_products WHERE productid = ?", array($productid));	
			$producttransferToWD = $adb->query_result($productResult, 0, 'transfer_to_wd');
			if($prevtransfertowd==0 && $producttransferToWD=='1' ){
				$adb->pquery("UPDATE vtiger_crmentity SET is_create = 1 where crmid = ?", array($productid));
			}	

			if($producttransferToWD==1)
			{
				/*2020-10-01 Thet Phyo Wai Transfter to WD Product MSCRM Ver 1.0 Start*/
				$vendorid = $_REQUEST['vendor_id'];
				$vendorResult = $adb->pquery("SELECT transfer_to_wd FROM vtiger_vendor WHERE vendorid = ?", array($vendorid));
				$transferToWD = $adb->query_result($vendorResult, 0, 'transfer_to_wd');

				if($transferToWD == 0){
					$adb->pquery("UPDATE vtiger_vendor SET transfer_to_wd = 1 WHERE vendorid = ?", array($vendorid));
					$adb->pquery("UPDATE vtiger_crmentity SET is_create = 1 WHERE crmid = ?", array($vendorid));
				}
				/*2020-10-01 Thet Phyo Wai Transfter to WD Product MSCRM Ver 1.0 End*/
			}
		}
		
		//*2020-11-24 Pyae Phyo Mon Update is_create=true after transfer, change transfer to WD field value Save MSCRM Ver 1.0 End*/


		$relatedToFieldResult = $adb->pquery("SELECT product_no,product_number from vtiger_products WHERE productid=?", array($productid));

		$fieldNo = $adb->query_result($relatedToFieldResult, 0, 'product_no');
		$fieldNoOne = $adb->query_result($relatedToFieldResult, 0, 'product_number');
		

		$already_exit = false;
		if($fieldNoOne != ""){
			$Result = $adb->pquery("SELECT productid FROM vtiger_products WHERE productid !=? AND product_number =?", array($productid,$fieldNoOne));

			if($adb->getRowCount($Result) > 0) $already_exit = true;
		}


		if($fieldNoOne == "" || $already_exit){
			$adb->pquery("update vtiger_products set product_number=? where product_no=?", array($fieldNo,$fieldNo));
		}
		// 2020-09-30 Myo Min Kyaw  UPdate the product no CRM Ver 1.0 add end
	...
	*/
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId) {
		...
		$rel_table_arr = Array("HelpDesk"=>"vtiger_troubletickets","Products"=>"vtiger_seproductsrel","Attachments"=>"vtiger_seattachmentsrel",
				...
				"Documents"=>"vtiger_senotesrel",'Assets'=>'vtiger_assets',"ContactProduct"=>"vtiger_contactproduct",); // 2020/11/24 sulatt product merge data lost problem MSCRM
		...
		$tbl_field_arr = Array("vtiger_troubletickets"=>"ticketid","vtiger_seproductsrel"=>"crmid","vtiger_seattachmentsrel"=>"attachmentsid",
				...
				"vtiger_senotesrel"=>"notesid",'vtiger_assets'=>'assetsid',"vtiger_contactproduct"=>"contactproduct_id"); // 2020/11/24 sulatt product merge data lost problem MSCRM
		...
		$entity_tbl_field_arr = Array("vtiger_troubletickets"=>"product_id","vtiger_seproductsrel"=>"crmid","vtiger_seattachmentsrel"=>"crmid",
				...
				"vtiger_senotesrel"=>"crmid",'vtiger_assets'=>'product',"vtiger_contactproduct"=>"product_name"); // 2020/11/24 sulatt product merge data lost problem MSCRM
	...
	function unlinkDependencies($module, $id) {
		...
		// 2020/12/9 sulatt Organization merge problem start
		//Backup ContactProduct-contact Relation
		$con_q = 'SELECT contactproduct_id FROM vtiger_contactproduct WHERE product_name = ?';
		$con_res = $this->db->pquery($con_q, array($id));
		if ($this->db->num_rows($con_res) > 0) {
			$con_ids_list = array();
			for($k=0;$k < $this->db->num_rows($con_res);$k++)
			{
				$con_ids_list[] = $this->db->query_result($con_res,$k,"contactproduct_id");
			}
			$params = array($id, RB_RECORD_UPDATED, 'vtiger_contactproduct', 'product_name', 'contactproduct_id', implode(",", $con_ids_list));
			$this->db->pquery('INSERT INTO vtiger_relatedlists_rb VALUES(?,?,?,?,?,?)', $params);
		}
		//Deleting ContactProduct-Contact Relation.
		$con_q = 'UPDATE vtiger_contactproduct SET product_name = 0 WHERE product_name = ?';
		$this->db->pquery($con_q, array($id));
		// 2020/12/9 sulatt Organization merge problem end
		...
		foreach($with_crmids as $with_crmid) {
			...
			if (in_array($with_module, array('Leads', 'Accounts', 'Contacts', 'Potentials', 'Products'))) {
				...
				if($adb->num_rows($query) == 0) {
					...
					//2020-11-25 Pyae Phyo Mon update the not need to transfer product part information field update MSCRM ver1.0 Start
					//2020/10/01 Myo Min Kyaw update the crmentity is_update field update MSCRM ver1.0 start	
						//$adb->query("UPDATE vtiger_crmentity set is_update = 1 where crmid = '$crmid'");
					//2020/10/01 Myo Min Kyaw update the crmentity is_update field update MSCRM ver1.0 end
					//2020-11-25 Pyae Phyo Mon update the not need to transfer product part information field update MSCRM ver1.0 End
				}
			} else {
