...
class Products_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel) {
        ...
        /*2020-11-09 Thet Phyo Wai Purchase Cost Currency Change Start*/
         foreach ($searchParams as $groupindex => $groupcolumns) {
			$filtercolumns = $groupcolumns['columns'];
			if (count($filtercolumns) > 0) {
				foreach ($filtercolumns as $index => $filter) {
					$nameComponents = explode(':', $filter['columnname']);
					if ($nameComponents[2] == 'purchase_cost'){
						$currentUser = Users_Record_Model::getCurrentUserModel();
						$purchaseCurrency = getCurrencySymbolandCRate($currentUser->get('currency_id'));
						$purchaseCost = (float)$filter['value'] * (float)$purchaseCurrency['rate'];
						$searchParams[$groupindex]['columns'][$index]['value'] = $purchaseCost;
					}
				}
			}
		}
		...
		$accountid = $this->get('relatedAccountId');
			if(!empty($contactid)){
			$listQuery .= " AND productid IN (SELECT product_name from vtiger_contactproduct INNER JOIN vtiger_crmentity ON vtiger_contactproduct.contactproduct_id = vtiger_crmentity.crmid WHERE contact_id = ".$contactid." AND vtiger_crmentity.deleted=0) ";
			}else if(!empty($accountid)){
				$listQuery .= " AND productid IN (SELECT product_name from vtiger_contactproduct INNER JOIN vtiger_crmentity ON vtiger_contactproduct.contactproduct_id = vtiger_crmentity.crmid WHERE organization_name = ".$accountid." AND vtiger_crmentity.deleted=0) ";
			}
		}
		//UPDATE END
		...
		/*2020-12-11 Thet Phyo Wai Related Product Popup Cannot go Next Page MSCRM Start*/
		/*if(($sourceModule !== 'PriceBooks' && $sourceField !== 'priceBookRelatedList')
				&& ($sourceModule !== 'Products' && $sourceField !== 'productsList')) {*/
		/*2020-12-11 Thet Phyo Wai Related Product Popup Cannot go Next Page MSCRM End*/
