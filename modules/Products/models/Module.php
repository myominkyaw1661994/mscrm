...
class Products_Module_Model extends Vtiger_Module_Model {
	...
	public function getQueryByModuleField($sourceModule, $field, $record, $listQuery) {
		...
		/*2020-09-17 Thet Phyo Wai Create Related Ticket MSCRM var 1.0 End*/
			...
			elseif ($sourceModule === 'HelpDesk') {
				$condition .= " AND vtiger_products.productid NOT IN (SELECT productid FROM vtiger_ticketsproductrel WHERE ticketid = ?) ";
			} 
			/*2020-09-17 Thet Phyo Wai Create Related Ticket MSCRM var 1.0 End*/
	...
	function get_product_tickets($recordModel, $relatedModuleModel) {
		$query = 'SELECT vtiger_troubletickets.ticketid, vtiger_troubletickets.ticket_no, vtiger_troubletickets.title, vtiger_troubletickets.priority, vtiger_crmentity.crmid, vtiger_crmentity.smownerid
			...
			INNER JOIN vtiger_products on vtiger_products.productid = vtiger_troubletickets.product_id
			LEFT JOIN vtiger_users ON vtiger_users.id=vtiger_crmentity.smownerid
			LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid '
			. Users_Privileges_Model::getNonAdminAccessControlQuery($relatedModuleModel->getName()) .'
			WHERE vtiger_products.productid = '.$recordModel->getId().' and vtiger_crmentity.deleted = 0';
