...
class Products_Record_Model extends Vtiger_Record_Model {
	...
	function getDetailsForInventoryModule($focus) {
		...
		$purchaseCurrencyId = $productDetails[1]['currency1'];
		$purchaseCurrency = getCurrencySymbolandCRate($purchaseCurrencyId);
		$purchaseCost = (float)$productDetails[1]['purchaseCost1'] * ((float)$currencyInfo['conversion_rate']/(float)$purchaseCurrency['rate']);
		...
		for ($i=1; $i<=count($productDetails); $i++) {
			...
			/*2020-10-20 Thet Phyo Wai Multi Currency Not Show MSCRM Ver 1.0 Start*/
			/*$productPrices = $this->getModule()->getPricesForProducts($currentUser->get('currency_id'), array($productId), $this->getModuleName());*/
			$productPrices = $this->getModule()->getPricesForProducts($currentUser->get('currency_id'), array($productId));
			/*2020-10-20 Thet Phyo Wai Multi Currency Not Show MSCRM Ver 1.0 Start*/
