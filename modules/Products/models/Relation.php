...
class Products_Relation_Model extends Vtiger_Relation_Model {
	...
	public function getQuery($recordModel, $actions=false){
		...
		/*2020-07-16 Thet Phyo Wai Related Document show inactive in custom module start*/
		if($relatedModuleName =='Documents' && $functionName=="get_attachments"){
			$query.=' AND vtiger_notes.filestatus = 1';
		}
		/*2020-07-16 Thet Phyo Wai Related Document show inactive in custom module end*/
	...
	public function deleteProductToProductRelation($sourceRecordId, $relatedRecordId) {
		...
		if(!empty($sourceRecordId) && !empty($relatedRecordId)){
			...
			/*2020/10/01 Myo Min Kyaw update the crmentity is_update field update and delete parent product MSCRM ver1.0 start*/
			/*$db->pquery('DELETE FROM vtiger_seproductsrel WHERE crmid = ? AND productid = ?', array($relatedRecordId, $sourceRecordId));*/
			$result = $db->query("SELECT crmid, productid FROM vtiger_seproductsrel WHERE productid='$sourceRecordId' AND crmid = '$relatedRecordId'");
			$related = $db->query_result($result, 0, 'crmid');
			$source = $db->query_result($result, 0, 'productid');
			if(($related == $relatedRecordId) && ($source == $sourceRecordId)){
				$db->pquery('DELETE FROM vtiger_seproductsrel WHERE crmid = ? AND productid = ?', array($relatedRecordId, $sourceRecordId));
				$db->query("UPDATE vtiger_crmentity set is_update = 1 where crmid = '$sourceRecordId'");	
			}else{
				$db->pquery('DELETE FROM vtiger_seproductsrel WHERE crmid = ? AND productid = ?', array($sourceRecordId, $relatedRecordId));
				$db->query("UPDATE vtiger_crmentity set is_update = 1 where crmid = '$relatedRecordId'");
				
			}
			/*2020/10/01 Myo Min Kyaw update the crmentity is_update field update and delete parent product MSCRM ver1.0 end*/
			return true;
		}
