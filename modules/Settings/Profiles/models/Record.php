		...
		if($moduleModel->isEntityModule() || $moduleModel->isUtilityActionEnabled()) {
			if($actionPermissions || $moduleModel->isUtilityActionEnabled()) {
				...
				if ($profileActionPermissions) {
                    ...
                    //$params = array(); // 2020/02/29 SuLatt Travel CRM Profile Error fix delete
                    $actionParams = array();	// 2020/02/29 SuLatt Travel CRM Profile Error fix add
					$actionsInsertQuery .= 'INSERT INTO vtiger_profile2standardpermissions(profileid, tabid, operation, permissions) VALUES ';
					foreach ($actionsIdsList as $actionId => $permission) {
						$actionEnabled = true;
						$permissionValue = $this->tranformInputPermissionValue($permission);
						$actionsInsertQuery .= "(?, ?, ?, ?)";
                        //array_push($params, $profileId, $tabId, $actionId, $permissionValue);  // 2020/02/29 SuLatt Travel CRM Profile Error fix delete

                        array_push($actionParams, $profileId, $tabId, $actionId, $permissionValue); // 2020/02/29 SuLatt Travel CRM Profile Error fix add
					...
					if ($actionsIdsList) {
						// $db->pquery($actionsInsertQuery, array()); // 2020/02/29 SuLatt Travel CRM Profile Error fix delete

						$db->pquery($actionsInsertQuery, array($actionParams)); // 2020/02/29 SuLatt Travel CRM Profile Error fix add
					}
                    ...
                    // $params = array(); // 2020/02/29 SuLatt Travel CRM Profile Error fix delete
                    $utilityParams  = array();	// 2020/02/29 SuLatt Travel CRM Profile Error fix add
					...
					foreach($utilityIdsList as $actionId => $permission) {
                        ...
                        array_push($utilityParams, $profileId, $tabId, $actionId, $permissionValue); // 2020/02/29 SuLatt Travel CRM Profile Error fix add
					...
					if ($utilityIdsList) {
						//$db->pquery($utilityInsertQuery, array());	// 2020/02/29 SuLatt Travel CRM Profile Error fix delete

						$db->pquery($utilityInsertQuery, array($utilityParams));	// 2020/02/29 SuLatt Travel CRM Profile Error fix add
					}
				}
