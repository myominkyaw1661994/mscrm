...
/*6-11-2020 Myo Min Kyaw Hide the Recycel Bin at Admin Manager Role MSCRM Ver 1.0 start */
	/**
	 * Function to check whether the user is an Admin Manager user
	 * @return <Boolean> true/false
	 */
	...
	public function isAdminManager(){
		...
		if($this->isAdminUser()){
			...
			$role = $db->pquery("SELECT rolename
			FROM vtiger_users
			JOIN vtiger_user2role ON vtiger_user2role.userid = vtiger_users.id
			JOIN vtiger_role ON vtiger_role.roleid = vtiger_user2role.roleid
			WHERE vtiger_users.id = ?", array($user_id));

			$result_role = $db->query_result($role, 0, 'rolename');

			if($result_role == "Admin Manager"){
				return true;
			}
			return false;
		}
...
/*6-11-2020 Myo Min Kyaw Hide the Recycel Bin at Admin Manager Role MSCRM Ver 1.0 end*/
	...
	public static function userLimitation() {
		$userModuleModel = Users_Module_Model::getCleanInstance('Users');
		// To check count existence in db
		return $userModuleModel->countUsers();
	}
