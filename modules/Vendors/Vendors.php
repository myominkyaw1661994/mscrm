...
class Vendors extends CRMEntity {
	...
	//2020-09-10 Pyae Phyo Mon Vendor Number Save at HIMS Ver 1.0 Start
	...
	{		
		...
		//*2020-10-08 Pyae Phyo Mon Update is_create=true after transfer, change transfer to WD field value Save MSCRM Ver 1.0 Start*/
		if($_REQUEST['action'] == "Save")
		{
			$prevtransfertowd = $_REQUEST['old_transfer_to_wd'];
			$vendorResult = $adb->pquery("SELECT transfer_to_wd FROM vtiger_vendor WHERE vendorid = ?", array($recordid));
			$transferToWD = $adb->query_result($vendorResult, 0, 'transfer_to_wd');
			if($prevtransfertowd==0 && $transferToWD=='1' ){
				$adb->pquery("UPDATE vtiger_crmentity SET is_create = 1 where crmid = ?", array($recordid ));
			}
		}
		//*2020-10-08 Pyae Phyo Mon Update is_create=true after transfer, change transfer to WD field value Save MSCRM Ver 1.0 End*/		


		$relatedToFieldResult =$adb->query('SELECT vendor_no,vendor_number,transfer_to_wd from vtiger_vendor where  vendorid='.$recordid);

		$fieldNo = $adb->query_result($relatedToFieldResult, 0, 'vendor_no');
		$fieldNumber = $adb->query_result($relatedToFieldResult, 0, 'vendor_number');
		$already_exit = false;
		if($fieldNumber != ""){
			$Result = $adb->pquery("SELECT vendorid FROM vtiger_vendor WHERE vendorid !=? AND vendor_number =?", array($recordid,$fieldNumber));

			if($adb->getRowCount($Result) > 0) $already_exit = true;
		}
		if($fieldNumber == "" || $already_exit)
		{
			$adb->pquery("update vtiger_vendor set vendor_number=? where vendor_no=?", array($fieldNo,$fieldNo));	
		}
	}
	//2020-09-10 Pyae Phyo Mon Vendor Number Save at HIMS Ver 1.0 End
