...
class Vtiger_ExportData_Action extends Vtiger_Mass_Action {
	...
	function getExportQuery(Vtiger_Request $request) {
		...
		switch($mode) {
			case 'ExportAllData'	:	if ($orderBy && $orderByFieldModel) {
										...
										/*2020-12-07 Thet Phyo Wai Update Export Data Sorting MSCRM Start*/
										else{
											$query .= ' ORDER BY modifiedtime DESC';
										}
										/*2020-12-07 Thet Phyo Wai Update Export Data Sorting MSCRM End*/
										break;
	...
	function sanitizeValues($arr){
		...
		foreach($arr as $fieldName=>&$value){
			...
			/*2020-12-10 Thet Phyo Wai Export Currency Name Not Show MSCRM Start*/
			elseif($uitype == 117 && $type == 'currencyList'){
				$result = $db->pquery('SELECT currency_name FROM vtiger_currency_info WHERE id = ?',array($value));
				$currency_array  = [];
				for ($i = 0; $i < $db->num_rows($result); $i++) {
					$currency_name = $db->query_result($result, $i, 'currency_name');
				}
				/*2021-01-14 Myo Min Kyaw Export Price Book Currency Problem MSCRM Ver 1.0 Start*/ 
				if($db->num_rows($result) != 0){
					$value = $currency_name;
				}
				/*2021-01-14 Myo Min Kyaw Export Price Book Currency Problem MSCRM Ver 1.0 end*/
	
			}
			/*2020-12-10 Thet Phyo Wai Export Currency Name Not Show MSCRM End*/
