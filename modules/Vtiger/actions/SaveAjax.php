			...
			foreach ($fieldModelList as $fieldName => $fieldModel) {
				if($fieldModel->isViewable()){
					...
					if ($fieldModel->getFieldDataType() == 'currency') {
						...
						if($request->get(action) == "SaveAjax" && $request->get(module) == "ContactProduct"){
							//2021-06-14 Phyo Phyo Win Change the Decimal 2 for contract price in Customer Product link MSCRM var 1.0 Start-
						    //$displayValue = Vtiger_Currency_UIType::transformDisplayValue($fieldValue,null,true,true);
							$value=str_replace(',', '', $fieldValue);
							$displayValue=round($value,2);
							//2021-05-14 Phyo Phyo Win Change the Decimal 2 for contract price in Customer Product link MSCRM var 1.0 End-
						}else{
