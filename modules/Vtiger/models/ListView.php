...
class Vtiger_ListView_Model extends Vtiger_Base_Model {
	...
	public function getListViewEntries($pagingModel) {
		...
		/*2020/09/29 Thet Phyo Wai Ticket Module Update Ver 1.0 Start*/
		//UPDATE START
		$accountid = $this->get('relatedAccountId');
		if($sourceModule === "HelpDesk" && $moduleName === "Contacts" && !empty($accountid)){
			$listQuery .= " AND accountid = ".$accountid." ";
		}

		$contactid = $this->get('relatedContactId');
		if($sourceModule === "HelpDesk" && $moduleName === "ContactProduct"){
		$productid = $this->get('relatedProductId');
			if(!empty($productid)) $listQuery .=" AND product_name = ".$productid;
			if(!empty($contactid)) $listQuery .=" AND contact_id = ".$contactid;
			if(!empty($accountid)) $listQuery .=" AND organization_name = ".$accountid;
			
		}
		//UPDATE END
		/*2020/09/29 Thet Phyo Wai Ticket Module Update Ver 1.0 End*/

		/*2020/10/22 Thet Phyo Wai Invoice Module Customize MSCRM Ver 1.0 Start*/
		if($sourceModule === "Invoice" && !empty($contactid) ){
			$result = $db->pquery("SELECT accountid FROM vtiger_contactdetails WHERE contactid = ? and accountid != 0", array($contactid));

			if($db->num_rows($result)){
				$accountId = $db->query_result($result, 0, 'accountid');
			
				if($accountId != "" || $accountId != 0){
					$listQuery .= " AND (vtiger_account.accountid = ".$accountId." OR vtiger_account.parentid = ".$accountId." OR vtiger_account.accountid = (SELECT parentid FROM vtiger_account WHERE accountid = ".$accountId.")) ";
				}
			}
		}
		/*2020/10/22 Thet Phyo Wai Invoice Module Customize MSCRM Ver 1.0 End*/


		/* 2021/06/25 Myo Min Kyaw Inactive Customer product link machine serial number show in ticket start */
		if($sourceModule == "HelpDesk" && $moduleName == "ContactProduct" && $this->get(src_field) == "machine_serial_no"){
			$listQuery .= " AND vtiger_contactproduct.customer_active=\"YES\"";
		}
		/* 2021/06/25 Myo Min Kyaw Inactive Customer product link machine serial number show in ticket start */
		...
		if(!empty($orderBy) && $orderByFieldModel) {
			if($orderBy == 'roleid' && $moduleName == 'Users'){
				...
				/*2020-10-21 Thet Phyo Wai Blank After Sort MSCRM Ver 1.0 Start*/
				/*$listQuery .= ' ORDER BY ? '.$sortOrder;
				array_push($paramArray, $queryGenerator->getOrderByColumn($orderBy));*/
				$listQuery .= ' ORDER BY '.$queryGenerator->getOrderByColumn($orderBy).' '.$sortOrder;
				/*2020-10-21 Thet Phyo Wai Blank After Sort MSCRM Ver 1.0 End*/
			}
