...
class Vtiger_Detail_View extends Vtiger_Index_View {
	...
	function __construct() {
		...
		$this->exposeMethod('getCompletedActivities');
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		/*2020-11-23 Thet Phyo Wai Permission for Edit MSCRM Ver 1.0 Start*/
		$currentUser = Users_Record_Model::getCurrentUserModel();
		$viewer->assign('IS_EDIT', FALSE);
		/*if( $moduleName == 'HelpDesk' && $recordModel->get('ticketstatus') == 'Closed' && !($currentUser->isAdminUser()) ){
			$viewer->assign('IS_EDIT', TRUE);
		}*/
		if( $moduleName == 'PurchaseOrder' && $recordModel->get('approve') == 1 && !($currentUser->isAdminUser()) ){
			$viewer->assign('IS_EDIT', TRUE);
		}
		if( $moduleName == 'Invoice' && $recordModel->get('invoicestatus') == 'Paid' && !($currentUser->isAdminUser()) ){
			$viewer->assign('IS_EDIT', TRUE);
		}
		/*2020-11-23 Thet Phyo Wai Permission for Edit MSCRM Ver 1.0 End*/
		...
		/*2020-07-01 Thet Phyo Wai icon not show correctly start*/
		...
		foreach ($menuGroupedByParent as $parentCategory => $menuList) {
			...
			if(count($menuList) > 0) {
				if(array_key_exists($moduleName, $menuList) && $parentCategory) {
					$moduleFound = true;
		...
		if($moduleFound) {
			$selectedMenuCategoryLabel = vtranslate('LBL_'.$selectedModuleMenuCategory, $moduleName);
		}else{
			$selectedMenuCategoryLabel = vtranslate($moduleName, $moduleName);
		}
		/*2020-07-01 Thet Phyo Wai icon not show correctly end*/
	...
	function showModuleSummaryView($request) {
		...
		/*2020-10-01 Thet Phyo Wai Completed Activity MSCRM Ver 1.0 Start*/
		$viewer->assign('COMPLETED_ACTIVITIES', $this->getCompletedActivities($request));
		/*2020-10-01 Thet Phyo Wai Completed Activity MSCRM Ver 1.0 End*/
		...
		/*2020-10-21 Thet Phyo Wai Permission for Edit MSCRM Ver 1.0 End*/
