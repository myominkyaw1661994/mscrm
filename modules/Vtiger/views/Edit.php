...
Class Vtiger_Edit_View extends Vtiger_Index_View {
	...
	// 2020-09-15 Pyae Phyo Mon disable field Ver 1.0 add start *}
	public function getIs_CreateUpdateProcess($id){

		$db = PearDatabase::getInstance();
		$relatedToFieldResult =$db->pquery("SELECT crmid,setype,is_create,is_update from vtiger_crmentity where crmid=?",array($id));

		$fieldValue = $db->query_result($relatedToFieldResult, 0, 'is_create');
		return $fieldValue;
	}
	// 2020-09-15 Pyae Phyo Mon disable field Ver 1.0 add end *}
	...
	public function process(Vtiger_Request $request) {
		...
		{
			$viewer->assign('IS_CREATE', $this->getIs_CreateUpdateProcess($record));
		}
		// 2020-09-15 Pyae Phyo Mon disable field Ver 1.0 add end *}
		...
		/*2020-11-24 Thet Phyo Wai Update Ticket Module for Edit MSCRM Start*/
		$viewer->assign('IS_EDIT', false);
		if($moduleName=="HelpDesk" && !empty($record)){
			$ticket_no = $this->record->get('ticket_no');
			$alreadyGenerated = HelpDesk_Module_Model::getGenerateInvoice($ticket_no);
			$viewer->assign('IS_EDIT', $alreadyGenerated);
		}
		/*2020-11-24 Thet Phyo Wai Update Ticket Module for Edit MSCRM End*/
